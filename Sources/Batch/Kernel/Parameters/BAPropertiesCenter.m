...
{
    NSDate *lastInstallDate = [[[NSFileManager defaultManager] attributesOfItemAtPath:NSTemporaryDirectory() error:nil] objectForKey:NSFileCreationDate];
...
- (NSString *)notifType
{
    ...
    if (settings != nil && settings.status != BANotificationAuthorizationStatusWaitingForValue) {
        ...
        NSUInteger alertTypes = BANotificationAuthorizationTypesAlert |
        ...
        [BALogger debugForDomain:@"Properties" message:@"Tried to read notificaiton authorization, but was nil or waiting for a value. Falling back on pre-iOS 10 API."];
    }
