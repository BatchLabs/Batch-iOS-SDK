...
+ (nullable NSData *)dataByGzipping:(nullable NSData *)data
{
    ...
    do {
        ...
        if (result != Z_OK && result != Z_STREAM_END) {
            [BALogger debugForDomain:LOCAL_DEBUG_DOMAIN message:@"Gzip error: deflating didn't return Z_OK | Z_STREAM_END, bailing."];
        }
    ...
    if (result != Z_STREAM_END) {
        // zlib isn't happy
        [BALogger debugForDomain:LOCAL_DEBUG_DOMAIN message:@"Gzip error: deflating didn't return Z_STREAM_END after completion"];
        return nil;
    }
...
+ (nullable NSData *)dataByGunzipping:(nullable NSData *)data
{
    ...
    do {
        ...
        if (result != Z_OK && result != Z_STREAM_END) {
            [BALogger debugForDomain:LOCAL_DEBUG_DOMAIN message:@"Gzip error: inflating didn't return Z_OK | Z_STREAM_END, bailing."];
        }
        // avail_out is the number of bytes that zlib didn't use in the buffer
        [outputData appendBytes:outputBuffer length:CHUNK_SIZE - zstream.avail_out];
    } while (zstream.avail_out == 0);
    ...
    if (result != Z_STREAM_END) {
        // zlib isn't happy
        [BALogger debugForDomain:LOCAL_DEBUG_DOMAIN message:@"Gzip error: inflating didn't return Z_STREAM_END after completion"];
        return nil;
    }
