...
+ (NSArray<BALocalCampaign*>*)parseCampaigns:(NSDictionary *)rawJson outPersistable:(NSDictionary **)persist error:(NSError**)error {
    ...
    BATJsonDictionary *json = [[BATJsonDictionary alloc] initWithDictionary:rawJson errorDomain:@"com.batch.module.localcampaigns.parser.error"];
    ...
    if (serverError != nil) {
        ...
        if (error) {
            if (errorDetails != nil) {
                *error = [BALocalCampaignsParser genericParsingErrorForReason:
                          [NSString stringWithFormat:@"Server returned an error. Code: %@, Message: '%@'", errorDetails[@"code"], errorDetails[@"message"]]];
            } else {
    ...
    for (NSObject *rawJsonContent in rawJsonContents) {
        if (![rawJsonContent isKindOfClass:[NSDictionary class]]) {
            [BALogger errorForDomain:@"Local Campaigns" message:@"A non-JSON Object was found in the 'contents' array of local campaigns (type: %@). Ignoring.", NSStringFromClass([rawJsonContent class])];
            continue;
        }
        
        BALocalCampaign *parsedCampaign = [self parseCampaign:(NSDictionary*)rawJsonContent error:&outErr];
        if (parsedCampaign == nil) {
            [BALogger errorForDomain:@"Local Campaigns" message:@"An error occurred while parsing a local campaign: %@. Ignoring.", [outErr localizedDescription]];
            continue;
        }
    ...
    [BALogger debugForDomain:@"Local Campaigns" message:@"Successfully parsed %lu campaigns", (unsigned long)[parsedCampaigns count]];
    
    if (persist != nil) {
        [BALogger debugForDomain:@"Local Campaigns" message:@"Persisting %lu campaigns", (unsigned long)[persistCampaigns count]];
        *persist = @{
...
+ (BALocalCampaign*)parseCampaign:(NSDictionary*)rawJson error:(NSError**)error {
    BATJsonDictionary *json = [[BATJsonDictionary alloc] initWithDictionary:rawJson errorDomain:@"com.batch.module.localcampaigns.parser.error.campaign"];
    ...
    campaign.devTrackingIdentifier = [json objectForKey:@"devTrackingId" kindOfClass:[NSString class] allowNil:YES error:&outErr];
    ...
    campaign.minimumAPILevel = [[json objectForKey:@"minimumApiLevel" kindOfClass:[NSNumber class] fallback:@0] integerValue];
    ...
    NSDictionary *startDateJson = [json objectForKey:@"startDate" kindOfClass:[NSDictionary class] allowNil:YES error:&outErr];
    ...
    campaign.minimumDisplayInterval = [[json objectForKey:@"minDisplayInterval" kindOfClass:[NSNumber class] fallback:@(60)] integerValue];
    
    campaign.persist = [[json objectForKey:@"persist" kindOfClass:[NSNumber class] fallback:@(YES)] boolValue];
    
    campaign.customPayload = [json objectForKey:@"customPayload" kindOfClass:[NSDictionary class] allowNil:YES error:&outErr];
...
+ (NSArray<id<BALocalCampaignTriggerProtocol>>*)parseTriggers:(NSArray*)rawJson error:(NSError**)error {
    ...
    for (NSDictionary *jsonTrigger in rawJson) {
        ...
        if (parsedTrigger == nil) {
            [BALogger debugForDomain:@"Local Campaigns" message:@"Could not parse trigger, skipping. Reason: %@", err ? err.localizedDescription : @"Unknown error"];
            continue;
        }
...
+ (id<BALocalCampaignTriggerProtocol>)parseTrigger:(NSDictionary*)rawJson error:(NSError**)error {
    ...
    NSString *type = [[json objectForKey:@"type" kindOfClass:[NSString class] allowNil:NO error:&outErr] uppercaseString];
    if (type == nil) {
        if (error) {
            *error = outErr;
        }
...
+ (id<BALocalCampaignOutputProtocol>)parseOutput:(NSDictionary*)rawJson error:(NSError**)error {
    BATJsonDictionary *json = [[BATJsonDictionary alloc] initWithDictionary:rawJson errorDomain:@"com.batch.module.inapp.parser.error.output"];
    ...
    if ([@"LANDING" isEqualToString:type]) {
        NSDictionary *payload = [json objectForKey:@"payload" kindOfClass:[NSDictionary class] allowNil:NO error:&outErr];
        if (payload == nil) {
            if (error) {
                *error = outErr;
            }
        ...
        BALocalCampaignLandingOutput *output = [[BALocalCampaignLandingOutput alloc] initWithPayload:payload error:&outErr];
        if (output == nil) {
            if (error) {
                *error = outErr;
            }
...
+ (NSDictionary<NSString*, NSObject*>*)parseServerError:(NSDictionary*)rawJson {
    BATJsonDictionary *json = [[BATJsonDictionary alloc] initWithDictionary:rawJson errorDomain:@"com.batch.module.inapp.parser.error.servererror"];
...
+ (BATZAwareDate*)parseDate:(NSDictionary*)rawJson error:(NSError**)error {
    BATJsonDictionary *json = [[BATJsonDictionary alloc] initWithDictionary:rawJson errorDomain:@"com.batch.module.inapp.parser.error.date"];
    ...
    return [BATZAwareDate dateWithDate:[NSDate dateWithTimeIntervalSince1970:timestamp/1000] relativeToUserTZ:[userTZ boolValue]];
}
