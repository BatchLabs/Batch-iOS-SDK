...
- (void)openDeeplink:(NSString *)deeplink inApp:(BOOL)inApp
{
    ...
    if (developerDelegate != nil) {
        [BALogger debugForDomain:LOGGER_DOMAIN message:@"Forwarding deeplink '%@' to developer implementation", deeplink];
        ...
        if (deeplinkURL != nil) {
            ...
            [BALogger debugForDomain:LOGGER_DOMAIN message:@"Tried to open deeplink '%@', but failed to convert it to a NSURL", deeplink];
        }
    }
}
...
- (void)excecuteStartWithAPIKey:(NSString *)key
{
    NSString *apiKey = [key stringByAddingPercentEncodingWithAllowedCharacters:NSCharacterSet.URLPathAllowedCharacterSet];
    ...
    if (e == nil)
    {
        ...
        // Versions management.
        NSString *storedVersion = [BAParameter objectForKey:kParametersSystemCurrentAppVersionKey fallback:currentSdkVersion];
...
{
    BOOL isRestartingFromWillEnterForeground = notification != nil && [UIApplicationWillEnterForegroundNotification isEqualToString:notification.name];
    ...
    BOOL isPotentiallyInBackgroundRefresh = !isRestartingFromWillEnterForeground && appState == UIApplicationStateBackground;
    
    // In UIScene apps, we need skip the first start WS as we can't know if the app is in background refresh or if the user really opened it
    // thus, all starts are reported as silent. We need to sacrifice silent starts for UIScene apps.
    ...
    if (@available(iOS 13.0, *)) {
        if ([BAApplicationLifecycle applicationUsesUIScene]) {
            // Check if it's a late call (like cordova, or just a bad integration), or if we're in application:didFinishLaunchingWithOptions:-ish
            if (appState == UIApplicationStateBackground && // Late starts (StateActive or StateInactive) should force a start
            ...
            // We can't rely on the UIScene state alone, as it's the same for first foreground and following ones on iOS 13
            // (iOS 14 fixes this, but who knows what 15 will break).
            // Note: this is a quick hack. We should rework this, and start to the session manager.
            // This solution looks like it might break at any moment, it already changed between iOS 13 and 14.
            //
            // We DO need to send the start, as we skipped it the first time.
            if (isRestartingFromWillEnterForeground &&
                appState == UIApplicationStateInactive) {
                [BALogger debugForDomain:LOGGER_DOMAIN message:@"Batch was asked to restart after UIApplicationWillEnterForegroundNotification but it looks like this is the app's first launch. Tracking start, but skipping other work."];
    ...
    if (e != NULL)
    {
        // If we're started but coming from WillEnterForeground, this means that we need to call the start webservice before returning.
        // This can happen when background refresh wakes up the app:
        // Batch will be started by the developer, but will never shut down because the app has been started in the background.
        // Therefore, we'll end up in this code path and no start will be tracked until the user closes and reopens the app: this is incorrect!
        // However users might rely on some stuff in the background so we can't just shut down the SDK
        // (actually we probably can because it's probably broken if the app is running in the background with Batch stopped,
        // but I'm pretty sure I'd face unforseen consequences if I tweaked this, so I'm not changing it just yet)
...
- (BOOL)openDeeplinkURLInAppIfPossible:(NSURL *)deeplinkURL
{
    @try {
        ...
        [BALogger debugForDomain:LOGGER_DOMAIN message:@"Opening deeplink '%@' using SFSafariViewController", deeplinkURL.absoluteString];
        
        return YES;
        
    } @catch (NSException *exception) {
        
        [BALogger debugForDomain:LOGGER_DOMAIN message:@"Deeplink '%@' is not supported by SFSafariViewController. Falling back on UIApplication.openURL", deeplinkURL.absoluteString];
...
- (void)checkForIncompatibilities
{
    ...
    if (NSClassFromString(@"FIRApp") != nil && [BAPushCenter instance].shouldSwizzle) {
        [BALogger publicForDomain:nil message:@"⚠️ Firebase has been detected in your application, but Batch's manual integration has NOT been enabled."];
        ...
        [BALogger publicForDomain:nil message:@"⚠️ More info about the manual integration here: https://batch.com/doc/ios/advanced/manual-integration.html"];
    }
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (![BABundleInfo isSharedGroupConfigured]) {
            [BALogger publicForDomain:nil message:@"⚠️ The App Group '%@' hasn't been configured. See the documentation for more info: https://doc.batch.com/ios/advanced/app-groups", [BABundleInfo sharedGroupId]];
        }
    });
}
