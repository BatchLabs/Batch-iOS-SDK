...
- (instancetype)init {
    self = [super init];
    if (self) {
        _zeroUUID = [[NSUUID alloc] initWithUUIDBytes:(uuid_t){0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}];
...
- (void)settingsMayHaveChanged {
    ...
    if (_previousAuthorizationStatus != newAuthorizationStatus ||
        ![self uuidMatchesPrevious:newTrackingId])
    {
...
- (nonnull NSDictionary*)statusDictionaryRepresentation
{
    ...
    return @{
        @"attid": attributionID,
        @"tath": @([self trackingAuthorizationStatus])
    };
}
        ...
        if ([class respondsToSelector:NSSelectorFromString(@"sharedManager")] == YES)
        {
            ...
            if (manager != nil)
            {
                if ([manager respondsToSelector:selector] == YES)
                {
                    ...
                    return isTrackingEnabled ? BATrackingAuthorizationStatusAuthorized : BATrackingAuthorizationStatusDenied;
                }
            }
        }
    }
