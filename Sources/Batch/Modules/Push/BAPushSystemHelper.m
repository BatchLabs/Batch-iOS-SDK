...
- (void)registerCategories:(NSSet*)categories
{
    ...
    if ([categories count] > 0)
    {
        ...
        {
            [BALogger debugForDomain:@"BAPushCompatUN" message:@"Converting instances of UIUserNotificationCategory to UNNotificationCategory"];
        ...
        {
            [BALogger publicForDomain:@"BatchPush" message:@"Provided categories set contains more than one kind of class, Batch will NOT register ANY actions, please only fill the set with UNNotificationCategory or UIUserNotificationCategory instances."];
        }
    }
    ...
    [[UNUserNotificationCenter currentNotificationCenter] requestAuthorizationWithOptions:options completionHandler:^(BOOL granted, NSError * _Nullable error) {
        [BAThreading performBlockOnMainThreadAsync:^{
            ...
            if (currentStatus == BatchPushNotificationSettingStatusUndefined) {
                [[NSNotificationCenter defaultCenter] postNotificationName:BatchPushUserDidAnswerAuthorizationRequestNotification
                                                                    object:nil
        ...
        if (granted && [BANotificationAuthorization applicationSettings] == BatchPushNotificationSettingStatusUndefined)
        {
...
- (NSSet<UNNotificationCategory*>*)convertLegacyCategoriesForSet:(NSSet<UIUserNotificationCategory*>*)legacyCategories
{
...
- (nonnull UNNotificationAction*)convertLegacyAction:(nonnull UIUserNotificationAction*)legacyAction
{
    ...
    {
        NSString *buttonTitle = [[legacyAction parameters] objectForKey:UIUserNotificationTextInputActionButtonTitleKey];
        if (buttonTitle == nil)
        {
