    ...
    while (!endReached)
    {
        ...
        if (c == ')' || c == ' ') {
            ...
            if (tokenAccumulator != nil) {
                ...
                if (tmpVal == nil) {
                    tmpVal = [self errorWithMessage:[NSString stringWithFormat:@"Unknown symbol '%@': It is not an operator, and it could not be converted to a number", tokenAccumulator]];
                }
    ...
    while (!endReached)
    {
        ...
        if (c == ']') {
            ...
            if ([stringValue isKindOfClass:[BALErrorValue class]]) {
                ...
                return [self errorWithPositionAndMessage:[NSString stringWithFormat:@"Internal parser error: value is not a string nor an error"]];
            }
        } else if (c != ' ') {
            return [self errorWithPositionAndMessage:[NSString stringWithFormat:@"Unexpected character '%c' in string arary, expected \" or ]", c]];
        }
    }
        ...
        if (isEscaping) {
            switch (c) {
                ...
                case '\'': // Actually '
                    ...
                    break;
                default:
                    return [self errorWithPositionAndMessage:[NSString stringWithFormat:@"Invalid escaped character: \\%c", c]];
            }
...
- (BALValue*)parseVariable
{
    ...
    while (!endReached)
    {
        ...
        if (c == TOKEN_DELIMITER_VARIABLE) {
            if ([accumulator stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet].length == 0)
            {
...
{
    return [BALErrorValue errorWithKind:BALErrorValueKindTypeParser message:[NSString stringWithFormat:@"Unexpected EOF. Expected: %@", expected]];
}
            
- (BALErrorValue*)errorWithPositionAndMessage:(NSString*)message
{
    return [BALErrorValue errorWithKind:BALErrorValueKindTypeParser message:[NSString stringWithFormat:@"At position %lu: %@", (unsigned long)_pos, message]];
}
