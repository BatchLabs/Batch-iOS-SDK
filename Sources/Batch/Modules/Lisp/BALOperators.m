...
- (void)setupOperators {
    ...
    [self addOperator:[[BALOperator alloc]
                          initWithSymbol:@"if"
                                 handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                     NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                   NSUInteger nbArgs = values.count;
                                   if (nbArgs < 2 || nbArgs > 3) {
                                       return
                                           [BALErrorValue errorWithKind:BALErrorValueKindTypeError
                                                                message:@"if: should be called with 2 or 3 arguments"];
                                   }
                                       ...
                                       if (![condition.value isKindOfClass:[NSNumber class]]) {
                                           return [BALErrorValue
                                               errorWithKind:BALErrorValueKindTypeInternal
                                                     message:@"if: internal consistency error: boolean value should "
                                                             @"have an underlying NSNumber"];
                                       }
                                       return [(NSNumber *)condition.value boolValue] ? trueValue : falseValue;
                                   } else {
                                       return [BALErrorValue
                                           errorWithKind:BALErrorValueKindTypeError
                                                 message:@"if: condition should be nil or a boolean value"];
                                   }
                                 }]];

    [self
        addOperator:[[BALOperator alloc] initWithSymbol:@"and"
                                                handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                                    NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                                  if ([values count] == 0) {
                                                  ...
                                                  for (BALPrimitiveValue *atomValue in values) {
                                                      ...
                                                      if (atomValue.type != BALPrimitiveValueTypeBool ||
                                                          ![value isKindOfClass:[NSNumber class]]) {
                                                          return [BALErrorValue
                                                              errorWithKind:BALErrorValueKindTypeError
                                                                    message:@"and: Cannot compare non boolean values"];
                                                      }
    ...
    [self addOperator:[[BALOperator alloc] initWithSymbol:@"or"
                                                  handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                                      NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                                    if ([values count] == 0) {
                                                        return [BALPrimitiveValue valueWithBoolean:true];
                                                    ...
                                                    for (BALPrimitiveValue *atomValue in values) {
                                                        ...
                                                        if (atomValue.type != BALPrimitiveValueTypeBool ||
                                                            ...
                                                            return [BALErrorValue
                                                                errorWithKind:BALErrorValueKindTypeError
                                                                      message:@"or: Cannot compare non boolean values"];
                                                        }
                                 ...
                                 handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                   ...
                                   if (firstAtomValue == nil) {
                                       return [BALErrorValue errorWithKind:BALErrorValueKindTypeInternal
                                                                   message:@"=: first value shouldn't be nil"];
                                   }
                                   ...
                                   for (int i = 1; i < values.count; i++) {
                                       ...
                                       if (atomValue == nil) {
                                           return [BALErrorValue
                                               errorWithKind:BALErrorValueKindTypeInternal
                                                     message:@"=: value can't be nil. Are we out of bounds?"];
                                       }
                                 ...
                                 handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                   ...
                                   if ([values count] != 1) {
                                       return [BALErrorValue errorWithKind:BALErrorValueKindTypeError
                                                                   message:@"not: only accepts one argument"];
                                   }

                                   BALPrimitiveValue *firstAtomValue = values[0];
                                   if (firstAtomValue.type != BALPrimitiveValueTypeBool) {
                                       return [BALErrorValue errorWithKind:BALErrorValueKindTypeError
                                                                   message:@"not: argument should be a boolean"];
                                   }

                                   id firstValue = firstAtomValue.value;
                                   if (![firstValue isKindOfClass:[NSNumber class]]) {
                                       return [BALErrorValue errorWithKind:BALErrorValueKindTypeInternal
                                                                   message:@"not: first value should be a NSNumber"];
                                   }
    ...
    [self addOperator:[[BALOperator alloc]
                          initWithSymbol:@">"
                                 handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                     NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                   return [self
                                       performNumberOperation:^BOOL(NSNumber *referenceValue, NSNumber *currentValue) {
    ...
    [self addOperator:[[BALOperator alloc]
                          initWithSymbol:@"contains"
                                 handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                     NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                   if ([values count] != 2) {
                                       return [BALErrorValue errorWithKind:BALErrorValueKindTypeError
                                                                   message:@"contains: only accepts two arguments"];
                                   }
                                   ...
                                   if (wantedSetPrimitive.type == BALPrimitiveValueTypeString) {
                                       wantedSetPrimitive = [BALPrimitiveValue
                                           valueWithStringSet:[NSSet setWithObject:wantedSetPrimitive.value]];
                                   }
                                   ...
                                   if (wantedSetPrimitive.type != BALPrimitiveValueTypeStringSet ||
                                       ...
                                       return [BALErrorValue
                                           errorWithKind:BALErrorValueKindTypeError
                                                 message:@"contains: all arguments should be string sets"];
                                   }
                                   ...
                                   if (![wantedSet isKindOfClass:[NSSet class]] ||
                                       ...
                                       return [BALErrorValue
                                           errorWithKind:BALErrorValueKindTypeInternal
                                                 message:@"contains: internal consistency error: all arguments should "
                                                         @"be of underlying type NSSet"];
                                   }
    ...
    [self addOperator:[[BALOperator alloc] initWithSymbol:@"lower"
                                                  handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                                      NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                                    return [self
                                                        performStringOperation:^NSString *(NSString *valueToTransform) {
    ...
    [self addOperator:[[BALOperator alloc] initWithSymbol:@"upper"
                                                  handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                                      NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                                    return [self
                                                        performStringOperation:^NSString *(NSString *valueToTransform) {
    ...
    [self addOperator:[[BALOperator alloc]
                          initWithSymbol:@"parse-string"
                                 handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                     NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                   if ([values count] != 1) {
                                       return [BALErrorValue
                                           errorWithKind:BALErrorValueKindTypeError
                                                 message:@"parse-string: only accepts a string argument"];
                                   }
    ...
    [self
        addOperator:[[BALOperator alloc]
                        initWithSymbol:@"write-to-string"
                               handler:^BALValue *(id<BALEvaluationContext> _Nonnull context,
                                                   NSArray<BALPrimitiveValue *> *_Nonnull values) {
                                 if ([values count] != 1) {
                                     return [BALErrorValue
                                         errorWithKind:BALErrorValueKindTypeError
                                               message:@"write-to-string: only accepts a single, non-set argument"];
                                 }
                                 ...
                                 switch (value.type) {
                                     ...
                                     case BALPrimitiveValueTypeBool:
                                         if (![valueToStringify isKindOfClass:[NSNumber class]]) {
                                             return [BALErrorValue
                                                 errorWithKind:BALErrorValueKindTypeInternal
                                                       message:@"write-to-string: internal consistency error: argument "
                                                               @"should be of underlying type NSNumber"];
                                         }
                                         return [BALPrimitiveValue
                                             valueWithString:((NSNumber *)valueToStringify).boolValue ? @"true"
                                                                                                      : @"false"];
                                     case BALPrimitiveValueTypeDouble:
                                         ...
                                         return [BALPrimitiveValue
                                             valueWithString:[self stringifyNumber:(NSNumber *)valueToStringify]];
...
- (BALValue *)performNumberOperation:(BOOL (^)(NSNumber *referenceValue, NSNumber *currentValue))operation
    ...
    if ([values count] < 2) {
        return [BALErrorValue errorWithKind:BALErrorValueKindTypeError
                                    message:[symbol stringByAppendingString:@": requires at least two arguments"]];
    }

    BALPrimitiveValue *referenceValue = values[0];
    if (referenceValue.type != BALPrimitiveValueTypeDouble) {
        return [BALErrorValue errorWithKind:BALErrorValueKindTypeError
                                    message:[symbol stringByAppendingString:@": arguments should be numbers"]];
    }
    ...
    if (![referenceNumber isKindOfClass:[NSNumber class]]) {
        return [BALErrorValue
            errorWithKind:BALErrorValueKindTypeInternal
                  message:[symbol
                              stringByAppendingString:@": consistency error: underlying types should be NSNumbers"]];
    }
    ...
    for (int i = 1; i < values.count; i++) {
        ...
        if (atomValue == nil) {
            return [BALErrorValue
                errorWithKind:BALErrorValueKindTypeInternal
                      message:[symbol stringByAppendingString:@": value can't be nil. Are we out of bounds?"]];
        }
        ...
        if (![atomValue.value isKindOfClass:[NSNumber class]]) {
            return
                [BALErrorValue errorWithKind:BALErrorValueKindTypeInternal
                                     message:[symbol stringByAppendingString:
                                                         @": consistency error: underlying types should be NSNumbers"]];
        }
