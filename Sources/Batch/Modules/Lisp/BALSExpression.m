...
- (nonnull BALValue *)reduce:(nonnull id<BALEvaluationContext>)context
{
    ...
    {
        return [BALErrorValue errorWithKind:BALErrorValueKindTypeError message:@"S-Expressions should have an operator as their first value"];
    }
    
    NSMutableArray<BALPrimitiveValue*> *arguments = [[NSMutableArray alloc] initWithCapacity:MAX(0, _values.count-1)];
    for (int i = 1; i < _values.count; i++)
    {
        ...
        if (val == nil)
        {
            return [BALErrorValue errorWithKind:BALErrorValueKindTypeInternal message:@"Unexpected nil value while reducing S-Expression"];
        }
        ...
        {
            return [BALErrorValue errorWithKind:BALErrorValueKindTypeInternal message:@"Error while reducing S-Expression: at this point, value should be a PrimitiveValue"];
        }
    ...
    if (operator == nil) {
        return [BALErrorValue errorWithKind:BALErrorValueKindTypeInternal message:@"Can't reduce a S-Expression with a nil operator"];
    }
