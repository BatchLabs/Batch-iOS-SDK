...
+ (NSString *)urlForWebserviceType:(NSString *)type userInfo:(NSDictionary *)info
{
    if ([WebserviceUnlockIdentifierUnlockAuto isEqualToString:type])
    {
        return [NSString stringWithFormat:@"%@/%@", kParametersUnlockAutoWebserviceURL, [[BACoreCenter instance].configuration developperKey]];
    }
    else if ([WebserviceUnlockIdentifierCallback isEqualToString:type])
    {
        return [NSString stringWithFormat:@"%@/%@", kParametersCBWebserviceURL, [[BACoreCenter instance].configuration developperKey]];
    }
    else if ([WebserviceUnlockIdentifierCodeCallback isEqualToString:type])
    {
        NSString *code = [info objectForKey:@"code"];
        return [NSString stringWithFormat:@"%@/%@/%@", kParametersCBCodeWebserviceURL, [[BACoreCenter instance].configuration developperKey], [BAStringUtils encodeString:code withEncoding:NSUTF8StringEncoding]];
    }
    else if ([WebserviceUnlockIdentifierCode isEqualToString:type])
    {
        NSString *code = [info objectForKey:@"code"];
        return [NSString stringWithFormat:@"%@/%@/%@", kParametersCodeWebserviceURL, [[BACoreCenter instance].configuration developperKey], [BAStringUtils encodeString:code withEncoding:NSUTF8StringEncoding]];
    }
    else if ([WebserviceUnlockIdentifierRestore isEqualToString:type])
    {
       return [NSString stringWithFormat:@"%@/%@", kParametersRestoreWebserviceURL, [[BACoreCenter instance].configuration developperKey]];
    }
...
+ (NSArray *)queriesForWebservice:(BAStandardWebservice *)webservice
{
    ...
    {
        BAWebserviceQueryUnlockAutomatic *autoQuery = [[BAWebserviceQueryUnlockAutomatic alloc] initWithIdentifier:[BARandom generateUUID]];
    ...
    else if ([WebserviceUnlockIdentifierCallback isEqualToString:webservice.type])
    {
        ...
        BAWebserviceQueryConditional *callbackQuery = [[BAWebserviceQueryConditional alloc] initWithIdentifier:[BARandom generateUUID] andConditions:condition];
    ...
    else if ([WebserviceUnlockIdentifierCodeCallback isEqualToString:webservice.type])
    {
        ...
        BAWebserviceQueryConditionalCode *codeCallbackQuery = [[BAWebserviceQueryConditionalCode alloc] initWithIdentifier:[BARandom generateUUID] code:code andConditions:condition];
    ...
    else if ([WebserviceUnlockIdentifierCode isEqualToString:webservice.type])
    {
        ...
        BAWebserviceQueryCode *codeQuery = [[BAWebserviceQueryCode alloc] initWithIdentifier:[BARandom generateUUID] andCode:code];
    ...
    {
        BAWebserviceQueryRestore *restoreQuery = [[BAWebserviceQueryRestore alloc] initWithIdentifier:[BARandom generateUUID]];
    ...
    if ([WebserviceUnlockIdentifierUnlockAuto isEqualToString:webservice.type])
    {
        for (BAWebserviceResponse *response in responses)
        {
            ...
            if ([response isMemberOfClass:[BAWebserviceResponseUnlockAutomatic class]] == YES)
            {
                ...
                {
                    // Send conditions.
                    __block BAStandardWebservice *conditionWebservice = [[BAStandardWebservice alloc] initWithType:@"callback" identifier:nil userInfo:@{@"conditions": conditions}];
            ...
            if ([response isMemberOfClass:[BAWebserviceResponseCode class]] == YES)
            {
                ...
                if (status == BAOfferCodeNeedConditions)
                {
                    ...
                    if ([BANullHelper isArrayEmpty:conditions] == NO)
                    {
                        ...
                        __block BAStandardWebservice *callbackWebservice = [[BAStandardWebservice alloc] initWithType:@"codecallback" identifier:nil userInfo:info];
