...
+ (void)setupUnlockWithDelegate:(id<BatchUnlockDelegate>)delegate
{
    if (delegate == nil)
    {
        ...
        [BALogger publicForDomain:@"Unlock" message:@"%@",[[BAErrorHelper errorInvalidUnlockDelegate] localizedDescription]];
    }
...
- (void)batchDidStart
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkAutomaticUnlocks) name:UIApplicationWillEnterForegroundNotification object:nil];
...
- (void)checkAutomaticUnlocks
{
    ...
    __block BAStandardWebservice *unlockWebservice = [[BAStandardWebservice alloc] initWithType:@"unlockauto" identifier:nil userInfo:@{}];
...
- (BOOL)handleURL:(NSURL *)url
{
    ...
    {
        [BALogger errorForDomain:ERROR_DOMAIN message:@"%@",[BAErrorHelper errorBatchStoppedOnRedeem].localizedDescription];
        return NO;
    }
    ...
    // Call webservice.
    __block BAStandardWebservice *webservice = [[BAStandardWebservice alloc] initWithType:@"code" identifier:code userInfo:@{@"code": code, @"external": @YES}];
...
- (void)redeemCode:(NSString *)code success:(BatchSuccess)successBlock failure:(BatchFail)failBlock
{
    ...
    if ([[BACoreCenter instance].status isRunning] == NO)
    {
        [BALogger publicForDomain:@"Unlock" message:@"%@",[[BAErrorHelper errorBatchStoppedOnRedeem] localizedDescription]];
    ...
    __block BAStandardWebservice *webservice = [[BAStandardWebservice alloc] initWithType:@"code" identifier:code userInfo:info];
...
- (void)restoreFeatures:(BatchRestoreSuccess)successBlock failure:(BatchFail)failBlock
{
    ...
    __block BAStandardWebservice *webservice = [[BAStandardWebservice alloc] initWithType:@"restore" identifier:nil userInfo:info];
...
- (NSString *)extractCodeFromURL:(NSURL *)url
{
    ...
    NSString *schemePattern = [BAParameter objectForKey:kParametersSchemeCodePatternKey fallback:kParametersSchemeCodePatternValue];
    
    NSError *error;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:schemePattern options:NSRegularExpressionCaseInsensitive error:&error];
    if ([BANullHelper isNull:error] == YES)
    {
        NSArray *values = [regex matchesInString:[url absoluteString] options:0 range:NSMakeRange(0, [url absoluteString].length)];
