...
+ (void)batchWillStart
{
    ...
    // This prevent the tracker to be subscribe many times to the events in case the developper call `startWithAPIKey:` many times.
...
- (void)trackLocation:(nonnull CLLocation*)location
{
    ...
    if (_lastTrackedLocationTimestamp == nil) {
        [BALogger debugForDomain:DEBUG_DOMAIN message:@"Tracking location because no previous location has been tracked"];
        shouldTrackLocation = YES;
    } else if ([currentDate timeIntervalSinceDate:_lastTrackedLocationTimestamp]*1000 >= LOCATION_UPDATE_MINIMUM_TIME_MS ) {
        [BALogger debugForDomain:DEBUG_DOMAIN message:@"Tracking location because the minimum time interval since the last sent update has passed"];
        shouldTrackLocation = YES;
    }
...
- (BOOL)internalTrackEvent:(BAEvent *)event
{
    ...
    [BALogger debugForDomain:NSStringFromClass([self class]) message:@"Tracking event: %@ with parameters: %@",name,[parameters description]];
