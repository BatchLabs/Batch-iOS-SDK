...
- (void)acceptPressed
{
    ...
    [_messagingAnalyticsDelegate messageButtonClicked:self.messageDescription ctaIndex:0 action:self.messageDescription.acceptCTA];
    
    // We don't need to handle BAMSGCTAActionKindClose since we did that earlier
    [BAMessagingCenter.instance performAction:self.messageDescription.acceptCTA source:self.messageDescription.sourceMessage actionIndex:0 messageIdentifier:self.messageDescription.sourceMessage.devTrackingIdentifier];
}
