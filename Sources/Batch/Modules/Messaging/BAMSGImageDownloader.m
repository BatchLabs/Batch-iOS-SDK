          ...
          completionHandler:(void (^ __nonnull)(NSData* _Nullable rawData, BOOL isGif, UIImage* _Nullable image, NSError* _Nullable error))completionHandler;
{
    ...
    if ([url isFileURL]) {
        ...
        if (filesystemRepresentation != NULL && filesystemRepresentation[0] != '\0') {
            ...
            NSString *localImagePath = [NSString stringWithCString:filesystemRepresentation encoding:NSUTF8StringEncoding];
            
            NSData *imageData = [NSData dataWithContentsOfFile:localImagePath];
            
            if (imageData == nil) {
                completionHandler(nil, false, nil, [NSError errorWithDomain:@"com.batch.ios.BAMSGImageDownloaderError"
                                                                       code:-5
    ...
    NSURLSessionDataTask *task = [session dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if ([response isKindOfClass:[NSHTTPURLResponse class]])
        {
            ...
            {
                completionHandler(nil, false, nil, error ? error : [NSError errorWithDomain:@"com.batch.ios.BAMSGImageDownloaderError"
                                                                                       code:-1
                                                                                   userInfo:@{
                                                                                       NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Server returned a non successful statuscode (%ld)", (long)httpResponse.statusCode],
            ...
            if (!data)
            {
                completionHandler(nil, false, nil, error ? error : [NSError errorWithDomain:@"com.batch.ios.BAMSGImageDownloaderError"
                                                                                   ...
                                                                                   userInfo:@{
                                                                                       ...
                                                                                       kBATMessagingCloseErrorCauseKey: @(BATMessagingCloseErrorCauseInvalidResponse)
                                                                                   }]);
                return;
            }
