...
@interface BAMSGInterstitialViewController () <BATGIFAnimatorDelegate>
{
    ...
    // Not the same as "shouldWaitForImage", as shouldWaitForImage controls the loader state while this controls if the hero space should be reserved
...
- (void)viewDidLoad {
    ...
    if (self.attachCTAsBottom && [self.ctas count] > 0)
    {
        ...
        [content setContentCompressionResistancePriority:UILayoutPriorityDefaultHigh forAxis:UILayoutConstraintAxisVertical];
...
- (void)setupAudioSession
{
    ...
    if (err)
    {
        [BALogger errorForDomain:@"BAMSGVideoView" message:@"Error while setting AVAudioSession to inactive: %@", [err localizedDescription]];
    }
    ...
    if (err)
    {
        [BALogger errorForDomain:@"BAMSGVideoView" message:@"Error while changing AVAudioSession category: %@", [err localizedDescription]];
...
- (void)tearDownAudioSession
{
    ...
    NSError *err = nil;
    // Devs that know how to use AVAudioSession and need it will have to set the session as active when the view is dismissed
    [[AVAudioSession sharedInstance] setActive:NO error:&err];
    if (err)
    {
        [BALogger errorForDomain:@"BAMSGVideoView" message:@"Error while setting AVAudioSession to inactive: %@", [err localizedDescription]];
...
- (void)didFinishLoadingHero:(nullable UIImage*)heroImage
{
    [BAThreading performBlockOnMainThread:^{
        // Prevents the loader from showing if the hero failed to load (nil image) but the view hasn't been constructed yet
...
- (void)didFinishLoadingGIFHero:(nullable NSData*)gifData
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        if (gifFile == nil) {
            [BALogger debugForDomain:@"Messaging" message:@"Could not load gif file: (%ld) %@", (long)(err ? err.code : 0), err ? err.localizedDescription : @"unknown"];
                   ...
                   fillHeightByDefault:(BOOL)defaultFillHeight
{
    ...
    if (wantsHeightFilled)
    {
        if (!addedSafeAreaConstraints) {
            [parentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-(%f)-[innerView]-(%f)-|", padding.top, padding.bottom]
                                                                               options:0
                                                                               metrics:nil
    ...
    else
    {
        ...
        [parentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-(>=%f@1000)-[innerView]-(>=%f@1000)-|", padding.top, padding.bottom]
                                                                           options:0
                                                                           metrics:nil
                                                                             views:NSDictionaryOfVariableBindings(innerView)]];
    ...
    if (addedSafeAreaConstraints) {
        if (fitToSafeArea) {
            [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"|-(>=%f)-[innerView]-(>=%f)-|", padding.left, padding.right]
                                                                                     options:0
                                                                                     metrics:nil
                                                                                       views:NSDictionaryOfVariableBindings(innerView)]];
        } else {
            [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"|-(%f@700)-[innerView]-(%f@700)-|", padding.left, padding.right]
                                                                                     options:0
                                                                                     metrics:nil
                                                                                       views:NSDictionaryOfVariableBindings(innerView)]];
        }
...
- (void)fillImageContainerViewWithHero
{
    ...
    if (shouldWaitForImage && !self.heroImage)
    {
        ...
        [heroImageContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(0)-[heroLoadingPlaceholder]-(0)-|"
                                                                                   options:0
                                                                                   metrics:nil
                                                                                     views:NSDictionaryOfVariableBindings(heroLoadingPlaceholder)]];
        
        [heroImageContainer addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(0)-[heroLoadingPlaceholder]-(0)-|"
                                                                                   options:0
                                                                                   metrics:nil
                                                                                     views:NSDictionaryOfVariableBindings(heroLoadingPlaceholder)]];
        
        heroActivityIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
...
- (void)setHeroContentBaseRules:(UIView*)heroContent rules:(BACSSRules*)rules
{
    ...
    NSArray<NSLayoutConstraint*>* margins = [NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"|-(%f)-[heroContent]-(%f)-|", margin.left, margin.right]
                                                                                              options:0
                                                                                              metrics:nil
                                                                                                views:NSDictionaryOfVariableBindings(heroContent)];
    
    margins = [margins arrayByAddingObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-(%f)-[heroContent]-(%f)-|", margin.top, margin.bottom]
                                                                                             options:0
                                                                                             metrics:nil
                                                                                               views:NSDictionaryOfVariableBindings(heroContent)]];
...
- (void)setupMainConstraintsWithHeroPortrait
{
    ...
    NSMutableArray *visualConstraints = [NSMutableArray arrayWithObjects:
                                         @"|-(0)-[heroImageContainer]-(0)-|",
                                         @"|-(0)-[content]-(0)-|",
                                         @"|-(0)-[ctasContainer]-(0)-|",
                                         nil];
    ...
    for (NSString *visualConstraint in visualConstraints)
    {
        [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:visualConstraint
                                                                                 options:0
                                                                                 metrics:nil
                                                                                   views:NSDictionaryOfVariableBindings(heroImageContainer, content, ctasContainer)]];
    }
...
- (void)setupMainConstraintsWithHeroLandscape
{
    ...
    NSMutableArray *visualConstraints = [NSMutableArray arrayWithObjects:
                                         @"V:|-(0)-[heroImageContainer]-(0)-|",
                                         @"V:|-(0)-[content]-(0)-[ctasContainer]-(0)-|",
                                         nil];
...
{
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(0)-[content]-(0)-[ctasContainer]-(0)-|"
                                                                      options:0
                                                                      metrics:nil
    ...
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(0)-[ctasContainer]-(0)-|"
                                                                      options:0
                                                                      metrics:nil
