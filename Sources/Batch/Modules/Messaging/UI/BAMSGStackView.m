...
- (void)addItem:(nonnull BAMSGStackViewItem*)item
{
    ...
    // Automatically add the next separator beforehand. We need to do that once the subview is already in the view though, or
    // else autolayout will break since the view isn't in the hierarchy yet
...
- (BAMSGStackViewItem*)separatorForItem:(BAMSGStackViewItem*)item
{
    ...
    {
        NSString *separatorID = [NSString stringWithFormat:@"%@-sep-%d", [self.delegate separatorPrefixForStackView:self], separatorCount];
...
{
    // We return a intrinsic width of 10, to help integration: adding "height: 10" and no width doesn't make the view appear,
    // which gets quite confusing: setting a default width helps figure out that the width rule is missing.
    // We return a 0 intrinsic height so that separators are hidden by default
