...
@interface BAMSGTapControl: UIControl @end // A simple control that dims a little upon touch
@interface BAMSGImagePresentationController: UIPresentationController @end
@interface BAMSGSlideInAnimationController: NSObject <UIViewControllerAnimatedTransitioning> @end
@interface BAMSGSlideOutAnimationController: NSObject <UIViewControllerAnimatedTransitioning> @end
...
- (void)viewDidLoad {
    ...
    [_imageView setImageURL:[NSURL URLWithString:self.message.imageURL] completion:^(UIImage *image, NSError *error) {
        if (!weakSelf) { return; }
        BAMSGImageViewController *strongSelf = weakSelf;
        
        if (error != nil) { // On error, dismiss the controller
            if (strongSelf) {
                [weakSelf.messagingAnalyticsDelegate message:strongSelf.message closedByError:[BATMessagingCloseErrorHelper guessErrorCauseForError:error]];
            }
            [weakSelf dismiss];
        } else {
            BAMSGImageViewController *strongSelf = weakSelf;
            ...
            if (CGSizeEqualToSize(weakSelf.message.imageSize, CGSizeZero) && !weakSelf.message.isFullscreen) { // Image ratio was unknown at display time.
...
- (void)setupConstraints
{
    ...
    } else
    {
        ...
        if (CGSizeEqualToSize(_message.imageSize, CGSizeZero)) { // we use a 3:2 portrait ratio while image is being downloaded
            imageRatio = 1.5;
        }
        ...
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|->=40@1000-[_contentView]->=40@1000-|" options:0 metrics:nil views:views]];
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-40@750-[_contentView]-40@750-|" options:0 metrics:nil views:views]];

        // Set height of _contentView
        [self.view addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationLessThanOrEqual toItem:self.view attribute:NSLayoutAttributeHeight multiplier:1 constant:-80]];
        ...
        NSLayoutConstraint *verticalContentViewConstraint = [NSLayoutConstraint
                                                            constraintWithItem:_contentView
        ...
        // Center close button to top right corner of content view
        [self.view addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.closeButton attribute:NSLayoutAttributeCenterX multiplier:1 constant:0]];
        [self.view addConstraint:[NSLayoutConstraint constraintWithItem:_contentView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.closeButton attribute:NSLayoutAttributeCenterY multiplier:1 constant:0]];
        
        _enteringBottomConstraint = [NSLayoutConstraint
                                     constraintWithItem:_contentView
                                     attribute:NSLayoutAttributeTop
                                     relatedBy:NSLayoutRelationEqual
                                     toItem:self.view
...
- (void)updateRatioConstraintWithRatio:(CGFloat)ratio
{
    ...
    _ratioConstraint = [NSLayoutConstraint
                        constraintWithItem:_contentView
...
- (void)imageTapped
{
    if (self.message.globalTapDelay > 0 && [BAUptimeProvider uptime] < (_lastViewAppearanceUptime + self.message.globalTapDelay) ) {
    ...
    if (self.message.globalTapAction != nil) {
        [[self _doDismissSelfModal] then:^(NSObject * _Nullable value) {
            [self.messagingAnalyticsDelegate messageGlobalTapActionTriggered:self.message action:self.message.globalTapAction];
            [BAMessagingCenter.instance performAction:self.message.globalTapAction source:self.message.sourceMessage actionIndex:BatchMessageGlobalActionIndex messageIdentifier:self.message.sourceMessage.devTrackingIdentifier];
        }];
    }
}
...
{
    return [[BAMSGImagePresentationController alloc] initWithPresentedViewController:presented presentingViewController:presenting];
}
...
- (void)animateTransition:(nonnull id<UIViewControllerContextTransitioning>)transitionContext {
    BAMSGImageViewController *toController = (BAMSGImageViewController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
...
- (void)animateTransition:(nonnull id<UIViewControllerContextTransitioning>)transitionContext {
    BAMSGImageViewController *fromController = (BAMSGImageViewController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
