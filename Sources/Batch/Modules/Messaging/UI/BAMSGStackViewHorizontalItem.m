                                ...
                                attachToParentBottom:(BOOL)attachToParentBottom
{
    ...
    // I wish it could be cleaner, but a remote controllable autolayout understandable by mere mortals is a nice challenge.
    ...
    for (NSString *rule in [rules allKeys])
    {
        ...
        {
            [target setContentHuggingPriority:MAX(1, MIN(1000, [value integerValue])) forAxis:UILayoutConstraintAxisHorizontal];
        }
        else if ([@"content-hug-v" isEqualToString:rule])
        {
            [target setContentHuggingPriority:MAX(1, MIN(1000, [value integerValue])) forAxis:UILayoutConstraintAxisVertical];
        }
        else if ([@"compression-res-h" isEqualToString:rule])
        {
            [target setContentCompressionResistancePriority:[value integerValue] forAxis:UILayoutConstraintAxisHorizontal];
        }
        else if ([@"compression-res-v" isEqualToString:rule])
        {
            [target setContentCompressionResistancePriority:[value integerValue] forAxis:UILayoutConstraintAxisVertical];
        }
        else
        {
            if (!isSeparatorView)
            {
                ...
                {
                    // This is only applied when it is the last view, since we would have to read the previous constraint and add it up
                    // Way too much hassle for what we want
    ...
    else
    {
        ...
        [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-(>=%f@1000)-[target]-(>=%f@1000)-|", margin.top, -margin.bottom]
                                                                                 options:0
                                                                                 metrics:nil
    ...
    NSLayoutConstraint *leftMargin = [NSLayoutConstraint constraintWithItem:target
    ...
    if (isSeparatorView)
    {
        // If we're a separator view, and our previous view was asked to be attach to its parent, attach ourselves instead of the original view
        // The previous view will be attached to the separator
    ...
    else
    {
        ...
        // Use the separator view to support a right margin. Separator views don't support margin, so we can do that for them
        // If we don't have a next view (which is usually a separator), this will not work. If there's one but it is not a separator
        // we're in for some undefined behaviour!
