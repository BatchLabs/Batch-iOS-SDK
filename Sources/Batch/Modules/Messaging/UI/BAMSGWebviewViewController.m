...
@interface BAMSGWebviewViewController () <WKNavigationDelegate, WKUIDelegate, UIContextMenuInteractionDelegate, BATWebviewJavascriptBridgeDelegate, BATWebviewBridgeLegacyWKHandlerWebViewSource>
...
- (instancetype)initWithMessage:(BAMSGMessageWebView *)message andStyle:(BACSSDocument*)style
{
    ...
    if (self) {
        ...
        _bridge = [[BATWebviewJavascriptBridge alloc] initWithMessage:message
                                                             delegate:self];
...
- (void)viewDidLoad {
    ...
    _webView = [[WKWebView alloc] initWithFrame:windowFrame
...
- (void)setupConstraints
{
    ...
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.closeButton attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeRight multiplier:1 constant:-15]];
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.closeButton attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.topLayoutGuide attribute:NSLayoutAttributeBottom multiplier:1 constant:10]];
    ...
    if (_message.developmentMode) {
        if (@available(iOS 13.0, *)) {
            ...
            UILongPressGestureRecognizer *longPressRecognizer = [[UILongPressGestureRecognizer alloc] initWithTarget:self
                                                                                                              action:@selector(didLongPressCloseButton:)];
...
- (UIMenu*)developmentContextMenu NS_AVAILABLE_IOS(13_0) {
    __weak BAMSGWebviewViewController* weakSelf = self;
    NSArray<UIAction *> *items = @[
        [UIAction actionWithTitle:BAMSGWebviewDevMenuReload
...
- (void)showDevelopmentError:(nonnull NSString*)errorMessage
                 sourceError:(nullable NSError*)sourceError {
    UIAlertController *alert = [UIAlertController
                                alertControllerWithTitle:@"WebView Error"
...
- (void)nativelyApplyViewport {
    ...
    if (@available(iOS 11.0, *)) {
        ...
        NSString* viewportExtractionJavascript = @"document.querySelector('meta[name=\"viewport\"]').getAttribute(\"content\")";
        [_webView evaluateJavaScript:viewportExtractionJavascript completionHandler:^(id _Nullable result, NSError * _Nullable error) {
            BOOL isViewportCover = false;
            if ([result isKindOfClass:NSString.class]) {
                NSString *stringResult = (NSString*)result;
                if ([stringResult rangeOfString:@"viewport-fit=cover" options:NSCaseInsensitiveSearch].location != NSNotFound) {
            ...
            if (strongSelf != nil) {
                strongSelf->_webView.scrollView.contentInsetAdjustmentBehavior = isViewportCover ? UIScrollViewContentInsetAdjustmentNever : strongSelf->_originalWebViewScrollBehaviour;
...
- (WKWebViewConfiguration*)webViewConfiguration {
    ...
    if (@available(iOS 14.0, *)) {
        [userContentController addScriptMessageHandlerWithReply:[[BATWebviewBridgeWKHandler alloc] initWithBridge:_bridge]
                                                   contentWorld:WKContentWorld.pageWorld
                                                           name:@"batchBridge"];
    } else {
        [userContentController addScriptMessageHandler:[[BATWebviewBridgeLegacyWKHandler alloc] initWithBridge:_bridge webViewProvider:self]
                                                  name:@"batchBridge"];
    }
...
- (void)loadPageWithCacheEnabled:(BOOL)cacheEnabled {
    [BALogger debugForDomain:BAMSGWebviewPublicLoggerDomain message:@"Loading (with cache: %d) URL: %@", cacheEnabled, self.message.url];
...
- (void)closeAfterError:(nullable NSError*)error {
    [self.messagingAnalyticsDelegate message:self.message closedByError:[BATMessagingCloseErrorHelper guessErrorCauseForError:error]];
    [self dismiss];
}
...
{
    [self performAction:@"batch.dismiss"
              arguments:@{}
            analyticsID:analyticsIdentifier];
}
          ...
          analyticsID:(NSString *)analyticsIdentifier
{
    ...
    if (idLength > 0 && idLength > 30) {
        [BALogger publicForDomain:@"Messaging" message:@"Could not track webview event: The analytics ID is invalid: it should be 30 characters or less. "
         ...
         analyticsID:(nullable NSString *)analyticsIdentifier
{
    ...
    [self performAction:@"batch.deeplink"
              arguments:deeplinkArgs
            analyticsID:analyticsIdentifier];
}
...
openInAppOverride:(nullable NSNumber *)openInAppOverride
   analyticsID:(nullable NSString *)analyticsIdentifier {
    ...
    [self openDeeplink:url
     openInAppOverride:openInAppOverride
           analyticsID:analyticsIdentifier];
}
...
decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if (navigationAction.targetFrame.isMainFrame) {
        ...
        if (shouldOverrideHandling) {
            ...
            [self openDeeplink:targetURL.absoluteString
             openInAppOverride:nil
                   analyticsID:nil];
            return;
        }
    }
...
decisionHandler:(void (^)(WKNavigationResponsePolicy))decisionHandler
{
    ...
    [BALogger debugForDomain:BAMSGWebviewPublicLoggerDomain message:@"WKNavigationDelegate - Decide policy for response (code: %ld, url: %@)", statusCode, httpResponse.URL];
    ...
    if (statusCode == 310 || statusCode >= 400) {
        ...
        _nextNavigationErrorOverride = [NSError errorWithDomain:WEBVIEW_ERROR_DOMAIN
                                                           code:1
                                                       userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"HTTP Error Code %ld", statusCode]}];
    }
...
{
    [BALogger debugForDomain:BAMSGWebviewPublicLoggerDomain message:@"WKNavigationDelegate - Failed provisional navigation"];
    [self handleNavigationError:error];
}
...
- (void)handleNavigationError:(NSError *)error
{
    ...
    [BALogger publicForDomain:BAMSGWebviewPublicLoggerDomain message:@"WebView was closed because of a navigation error"];
    ...
    if (self.message.developmentMode) {
        NSString *errorMessage = [NSString stringWithFormat:@"The WebView encountered an error and will be closed.\nThis error will only be shown during development.\n\nCause: %@", error.localizedDescription];
...
- (void)webViewWebContentProcessDidTerminate:(WKWebView *)webView
{
    ...
    if (self.message.developmentMode) {
        NSString *errorMessage = [NSString stringWithFormat:@"The WebView crashed and will be closed.\nThis error will only be shown during development."];
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler
{
    ...
    [controller addAction:[UIAlertAction actionWithTitle:[BAMSGViewToolbox localizedStringUsingUIKit:@"OK"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        completionHandler();
    }]];
...
- (void)webView:(WKWebView *)webView runJavaScriptConfirmPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(BOOL))completionHandler
{
    ...
    UIAlertController *controller = [UIAlertController alertControllerWithTitle:message message:nil preferredStyle:UIAlertControllerStyleAlert];
    [controller addAction:[UIAlertAction actionWithTitle:[BAMSGViewToolbox localizedStringUsingUIKit:@"OK"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        completionHandler(true);
    }]];
    [controller addAction:[UIAlertAction actionWithTitle:[BAMSGViewToolbox localizedStringUsingUIKit:@"Cancel"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        completionHandler(false);
    }]];
    [self presentViewController:controller animated:true completion:nil];
}
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString * _Nullable))completionHandler
{
    ...
    UIAlertController *controller = [UIAlertController alertControllerWithTitle:prompt message:nil preferredStyle:UIAlertControllerStyleAlert];
