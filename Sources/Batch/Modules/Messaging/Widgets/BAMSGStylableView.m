    ...
    for (NSString *rule in [rules allKeys])
    {
        ...
        else if ([@"background" isEqualToString:rule])
        {
            ...
            else if ([value hasPrefix:@"linear-gradient("] && [value hasSuffix:@")"])
            {
                ...
                float gradiantAngle = [[arguments[0] stringByReplacingOccurrencesOfString:@"deg" withString:@""] floatValue];
                
                NSMutableArray *colors = [NSMutableArray new];
                NSMutableArray *locations = [NSMutableArray new];
                for (int i = 1; i < [arguments count]; i++)
                {
                    NSString *argument = [arguments[i] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
                ...
                [(id<BAMSGGradientBackgroundProtocol>)view setBackgroundGradient:gradiantAngle colors:colors locations:locations];
            }
        }
        ...
        {
            [view setContentHuggingPriority:MAX(1, MIN(1000, [value integerValue])) forAxis:UILayoutConstraintAxisHorizontal];
        }
        else if ([@"content-hug-v" isEqualToString:rule])
        {
            [view setContentHuggingPriority:MAX(1, MIN(1000, [value integerValue])) forAxis:UILayoutConstraintAxisVertical];
        }
        else if ([@"compression-res-h" isEqualToString:rule])
        {
            [view setContentCompressionResistancePriority:[value integerValue] forAxis:UILayoutConstraintAxisHorizontal];
        }
...
+ (nullable UIColor*)colorFromValue:(nonnull NSString*)value
{
    ...
    else if (value)
    {
        ...
        {
            NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:[UIColor methodSignatureForSelector:colorSelector]];
...
+ (nullable UIFont*)fontFromRules:(nonnull BACSSRules*)rules baseFont:(UIFont*)baseFont baseBoldFont:(UIFont*)baseBoldFont
{
    ...
    else
    {
        ...
        UIFont *guessedFont = [UIFont fontWithDescriptor:[[labelFont fontDescriptor] fontDescriptorWithSymbolicTraits:traits] size:finalFontSize];
