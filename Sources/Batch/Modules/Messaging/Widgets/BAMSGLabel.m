...
- (void)applyRules:(nonnull BACSSRules*)rules
{
    ...
    UIFont *customFont = [BAMSGStylableViewHelper fontFromRules:rules baseFont:sBAMSGLabelFontOverride baseBoldFont:sBAMSGLabelBoldFontOverride];
    if (customFont)
    {
...
- (void)setText:(NSString *)text transforms:(NSArray<BATTextTransform*>*)transforms
{
    ...
    NSMutableAttributedString *attText = [[NSMutableAttributedString alloc] initWithString:text attributes:@{NSFontAttributeName: self.font}];
    ...
    if (letterSpacing != 0)
    {
        [attText addAttribute:NSKernAttributeName
                        value:@(letterSpacing)
                        range:NSMakeRange(0, [text length])];
    }
...
- (void)applyTransforms:(NSArray<BATTextTransform*>*)transforms toAttributedString:(NSMutableAttributedString*)attributedString
{
    ...
    for (BATTextTransform *transform in transforms.reverseObjectEnumerator) {
        ...
        if ((modifiers & BATTextModifierBold) ||
            (modifiers & BATTextModifierItalic) ||
            (modifiers & BATTextModifierSmallerFont) ||
            (modifiers & BATTextModifierBiggerFont)) {
            ...
            [attributedString addAttribute:NSFontAttributeName value:[self fontVariantForTraits:uiFontTraits size:fontSize] range:transform.range];
        }
        
        if (modifiers & BATTextModifierUnderline) {
            [attributedString addAttribute:NSUnderlineStyleAttributeName value:@(NSUnderlineStyleSingle) range:transform.range];
        }
        
        if (modifiers & BATTextModifierStrikethrough) {
            // Yes, Strikethrough reuses the underline styling enum
            [attributedString addAttribute:NSStrikethroughStyleAttributeName value:@(NSUnderlineStyleSingle) range:transform.range];
        }
        ...
        if (modifiers & BATTextModifierSpan) {
            ...
            UIColor *backgroundColor = [BAMSGStylableViewHelper colorFromValue:transform.attributes[@"background-color"]];
            if (backgroundColor != nil) {
                [attributedString addAttribute:NSBackgroundColorAttributeName value:backgroundColor range:transform.range];
            }
        }
    }
}
