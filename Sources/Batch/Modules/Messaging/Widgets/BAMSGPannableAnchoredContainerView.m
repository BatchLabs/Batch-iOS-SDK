                             ...
                             animations:^{
                                 ...
                                 if (self.biggestUserVisibleView) {
                                     ...
                                     if (self.verticalAnchor == BAMSGPannableAnchoredContainerVerticalAnchorTop) {
                                         // Straightforward here. Just take the biggest user visible view's Y position, and add the view height
                                 ...
                                 // Safeguard
                                 if (heightToHide <= 0) {
                                     [BALogger debugForDomain:@"BAMSGPannableContentView" message:@"Negative dismiss height calculation. Something is wrong with its calculation: falling back."];
...
- (BOOL)shouldDismissForPanEnd:(UIPanGestureRecognizer*)recognizer
{
    if (self.biggestUserVisibleView) {
        ...
        float pixelThreshold = self.biggestUserVisibleView.bounds.size.height * DISMISS_THRESHOLD_TRANSLATE_HEIGHT_RATIO;
