...
- (instancetype)initWithDelgate:(id<BatchMessagingDelegate>)delegate
{
    ...
    else
    {
        ...
        if (delegate)
        {
            [BALogger publicForDomain:@"Messaging" message:@"%@",[[BAErrorHelper errorInvalidMessagingDelegate] localizedDescription]];
        }
    }
...
- (BOOL)batchMessageDidTriggerAction:(BatchMessageAction * _Nonnull)action messageIdentifier:(NSString *)identifier actionIndex:(NSInteger)index
{
...
{
    if ([_delegate respondsToSelector:@selector(batchWebViewMessageDidTriggerAction:messageIdentifier:analyticsIdentifier:)])
    {
        [BAThreading performBlockOnMainThreadAsync:^{
