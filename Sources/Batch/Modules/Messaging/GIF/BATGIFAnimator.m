...
@interface BATGIFAnimator ()
{
    ...
    NSTimeInterval _elapsedTimeSinceLastFrame; // Count the elapsed time in between each frame. Even if a frame hasn't been displayed (because the decoder might be running late), we still increment this, so that we can skip problematic frames.
    BOOL _needsDisplayWhenPossible; // Flag indicating that we need to display the current frame's image when possible. This allows images to be decoded too late, as they will be reattempted at the next CADisplayLink tick
...
- (void)startAnimating
{
    ...
    if (!_displayLink) {
        ...
        _displayLink = [CADisplayLink displayLinkWithTarget:[[BATGIFDisplayLinkProxy alloc] initWithAnimator:self] selector:@selector(displayDidRefresh:)];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    if (_needsDisplayWhenPossible) {
        ...
        if (currentFrameImage != nil) {
            ...
            [BALogger debugForDomain:@"GIF" message:@"Expected frame %lu to be there, but it's not.", (unsigned long)_currentFrameIndex];
        }
    }
    
    // While-loop first inspired by & good Karma to: FLAnimatedImageView and https://github.com/ondalabs/OLImageView/blob/master/OLImageView.m
    // What is basically does, is increment the frames until we caught up.
    // Skipping too many frames might put the cache in a tough situation, so warn the file to warm up the next ones
