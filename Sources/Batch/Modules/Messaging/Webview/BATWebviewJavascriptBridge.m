...
/**
 ...
 Allows for different iOS 13- and iOS 14+ message implementations: iOS 14 supports replying directly from the message handler without
having to eval anything
 */
...
/**
 ...
 This method does all of the heavy lifting (argument checking, error handling): just give the bridge the result or error of this promise.
 */
- (nonnull BAPromise<NSString*>*)executeBridgeMethod:(nullable NSString*)method arguments:(nullable NSDictionary*)rawJSONArguments
{
    ...
    [internalPromise catch:^(NSError * _Nullable error) {
        ...
        if (error.code == BATWebviewJavascriptBridgeErrorCodePublicError) {
            ...
            [BALogger errorForDomain:LOGGER_DOMAIN message:@"Bridge couldn't execute unknown method '%@'", error.userInfo[@"methodName"]];
            outerError = [self makePublicError:error.localizedDescription];
        } else {
            [BALogger errorForDomain:LOGGER_DOMAIN message:@"Bridge raised an error, but could not be translated to a public error: %@", error.localizedDescription];
...
/**
 ...
 This allows for the public api boundary to easily rewrite all errors, and for other uses/tests of this method to easily get the "real" errors.
 */
- (nonnull BAPromise<NSString*>*)internalExecuteMethod:(nullable NSString*)method arguments:(nullable NSDictionary*)args
{
    if ([BANullHelper isStringEmpty:method]) {
        return [BAPromise rejected:[self makeInternalError:BATWebviewBridgeInternalErrorCodeMethodParsingFailure
    ...
    return [self resultProviderForMethod:method arguments:[[BATJsonDictionary alloc] initWithDictionary:args errorDomain:BRIDGE_ERROR_DOMAIN]];
}

// Returns the implementation
// All implementations should return a BAPromise
- (nonnull BAPromise<NSString*>*)resultProviderForMethod:(nonnull NSString*)method arguments:(nonnull BATJsonDictionary*)arguments
{
...
- (BAPromise<NSString*>*)attributionID
{
    ...
    if ([BANullHelper isStringEmpty:identifier]) {
        [promise reject:[self makePublicError:@"Not available: User denied permission to use it, application cannot use it due to missing framework or it was disabled in Batch's configuration."]];
    } else {
...
- (BAPromise<NSString*>*)performAction:(nonnull BATJsonDictionary*)arguments
{
    ...
    outErr = nil;
    NSDictionary *actionArgs = [arguments objectForKey:@"args" kindOfClass:NSDictionary.class allowNil:false error:&outErr];
    if (actionArgs == nil) {
...
- (nonnull NSError*)makeInternalError:(BATWebviewBridgeInternalErrorCode)code reason:(nonnull NSString*)reason underlyingError:(nullable NSError*)underlyingError
{
...
- (nonnull NSError*)makeUnknownMethodError:(nonnull NSString*)methodName
{
    return [NSError errorWithDomain:BRIDGE_ERROR_DOMAIN
                           ...
                           userInfo:@{
                               ...
                               NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Unimplemented native method '%@'. Is the native SDK too old?", methodName]
                           }];
}
