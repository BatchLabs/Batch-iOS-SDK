...
- (void)userContentController:(nonnull WKUserContentController *)userContentController
    ...
    if (![body isKindOfClass:NSDictionary.class]) {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Cannot decode taskID (missing body): webview cannot be called back"];
        return;
    }
    ...
    if (taskID < 0) {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Cannot decode taskID (invalid task id): webview cannot be called back"];
        return;
    }
    ...
    [self userContentController:userContentController
        ...
        NSString *callbackEval = [self javascriptCallbackForTaskID:taskID
                                                             error:errorMessage
                                                            result:reply];
...
- (NSString*)javascriptCallbackForTaskID:(NSInteger)taskID error:(nullable NSString*)error result:(nullable NSString*)result
{
    ...
    if (outErr != nil) {
        [BALogger errorForDomain:@"WebviewBridge" message:@"Could not serialize bridge json response object: %@", outErr.localizedDescription];
    }
    if (jsonResponse == nil) {
        jsonResponse = @"{'error': 'Unknown error (json)'}";
    }
    
    return [NSString stringWithFormat:@"window.batchInAppSDK.__onWebkitCallback(%ld, %@);",
            taskID,
            jsonResponse];
}
