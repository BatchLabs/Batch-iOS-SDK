...
- (NSString*)description
{
    return [NSString stringWithFormat:@"CSSDocument: Media queries:\r%@\r\rRulesets:\r%@", self.mediaQueries, self.rulesets];
}

- (NSString*)debugDescription
{
    return [[NSString stringWithFormat:@"CSSDocument: Media queries:\r%@\r\rRulesets:\r%@", self.mediaQueries, self.rulesets] stringByReplacingOccurrencesOfString:@"\\n" withString:@"\r"];
}
...
- (NSArray<BACSSDeclaration*>*)rulesForNode:(BACSSDOMNode*)node inRuleSets:(NSArray<BACSSRuleset*>*)rulesets
{
    ...
    for (BACSSRuleset *ruleset in rulesets)
    {
        ...
        if (![node matchesSelector:ruleset.selector]) { continue; }
...
+ (NSRegularExpression*)screenMediaQueryMatcher
{
    ...
    dispatch_once(&onceToken, ^{
        NSError *err = nil;
        screenMediaQueryRegexp = [NSRegularExpression regularExpressionWithPattern:SCREEN_MEDIA_QUERY_REGEXP options:NSRegularExpressionCaseInsensitive error:&err];
        
        if (err)
        {
            [BALogger debugForDomain:@"BACSS" message:@"Error while creating SCREEN_MEDIA_QUERY_REGEXP, @media queries will not work. Error: %@", [err localizedDescription]];
        }
    });
...
- (BOOL)environmentMatchesQuery:(NSString*)query
{
    ...
    if (!CGSizeEqualToSize(_viewSize, CGSizeZero))
    {
        ...
        if (screenRegexp)
        {
            NSTextCheckingResult* result = [screenRegexp firstMatchInString:query options:0 range:NSMakeRange(0, [query length])];
            if (result.numberOfRanges == 5)
            {
                ...
                return [self environmentMatchesRequiredSize:sizeString platform:platform minMax:minMax dimension:dimension];
            }
        }
    }
...
- (BOOL)environmentMatchesRequiredSize:(NSString*)sizeString platform:(NSString*)platform minMax:(NSString*)minMax dimension:(NSString*)dimension
{
