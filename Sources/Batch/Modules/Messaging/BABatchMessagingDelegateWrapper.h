...
/**
 ...
 Calls to these methods are safe, no matter whether there is a delegate or not, and if it implements the required methods or not.
 
 Methods usually returning void will return BOOL, indicating if the delegate implemented them and the invocations were forwared to it.
 */
...
- (BOOL)batchMessageDidTriggerAction:(nonnull BatchMessageAction *)action messageIdentifier:(nullable NSString *)identifier actionIndex:(NSInteger)index;
...
- (BOOL)batchMessageWasCancelledByError:(NSString* _Nullable)messageIdentifier NS_SWIFT_NAME(batchMessageWasCancelledByError(_:));
