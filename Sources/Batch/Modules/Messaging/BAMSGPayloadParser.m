                                 ...
                                 bailIfNotAlert:(BOOL)bailNotAlert
{
    ...
    {
        message = [BAMSGPayloadParser parseAlertMessage:rawMessage.messagePayload withBaseObject:(BAMSGMessageAlert*)message];
    }
    else if ([message isKindOfClass:[BAMSGMessageInterstitial class]])
    {
        ...
        message = [BAMSGPayloadParser parseUniversalMessage:rawMessage.messagePayload withBaseObject:(BAMSGMessageInterstitial*)message];
    }
    else if ([message isKindOfClass:[BAMSGMessageBanner class]])
    {
        ...
        message = [BAMSGPayloadParser parseBannerMessage:rawMessage.messagePayload withBaseObject:(BAMSGMessageBanner*)message];
    }
    else if ([message isKindOfClass:[BAMSGMessageModal class]])
    {
        ...
        message = [BAMSGPayloadParser parseModalMessage:rawMessage.messagePayload withBaseObject:(BAMSGMessageModal*)message];
    }
    else if ([message isKindOfClass:[BAMSGMessageImage class]])
    {
        ...
        message = [BAMSGPayloadParser parseImageMessage:rawMessage.messagePayload withBaseObject:(BAMSGMessageImage * )message];
    }
    else if ([message isKindOfClass:[BAMSGMessageWebView class]])
    {
        ...
        message = [BAMSGPayloadParser parseWebViewMessage:rawMessage.messagePayload withBaseObject:(BAMSGMessageWebView * )message];
    }
    else
    {
...
+ (BAMSGMessage* _Nullable)parseBaseMessage:(NSDictionary* _Nonnull)userData
{
    ...
    {
        [BAMSGPayloadParser printGenericDebugLog:[NSString stringWithFormat:@"minapi too high for this SDK. Got %ld, current SDK messaging API level: %u", (long)minimumMessagingAPIVersionInteger, BAMessagingAPILevel]];
        [BALogger publicForDomain:@"Messaging" message:@"This SDK is too old to display this message. Please update it."];
        return nil;
    }
        ...
        if ([bodyHtml isKindOfClass:[NSString class]])
        {
            ...
            if (parseErr == nil) {
                ...
                if (unstyledText.length == 0 && baseMessage.bodyText.length > 0) {
                    [BAMSGPayloadParser printGenericDebugLog:@"'body_html' is empty once parsed, but 'body' isn't: falling back on 'body'"];
                }
                ...
                [BALogger debugForDomain:LOGGER_DOMAIN message:@"Could not parse 'body_html': %@", parseErr.localizedDescription];
            }
        }
        else
        {
            // Not a fatal error, view controllers will fallback
            [BAMSGPayloadParser printGenericDebugLog:@"'bodyHtml' is not nil but isn't an NSString: falling back on 'body'"];
        }
    }
                                   ...
                                   withBaseObject:(BAMSGMessageAlert* _Nonnull)alert
{
    ...
    if ([alert.titleText stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length == 0 &&
        [alert.bodyText stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length == 0 )
    {
        [BAMSGPayloadParser printGenericDebugLog:@"Alert: 'title' and 'body' are both empty or null. At least one must be filled."];
        return nil;
    }
                                           ...
                                           withBaseObject:(BAMSGMessageInterstitial* _Nonnull)universal
{
    ...
    if (heroSplitRatio != nil)
    {
        ...
        {
            [BAMSGPayloadParser printGenericDebugLog:@"Universal: 'hero_split_ratio' is less or equal than 0 or greater than or equal to 1. Ignoring"];
            heroSplitRatio = nil;
        }
    }
 ...
 */
+ (BOOL)parseBaseBannerMessage:(NSDictionary* _Nonnull)userData
                    intoObject:(BAMSGMessageBaseBanner* _Nonnull)banner
{
    if (!banner)
    {
        return false;
    ...
    if (ctaDirection != nil)
    {
        ...
        if ([@"h" isEqualToString:ctaDirection]) {
            ...
            [BAMSGPayloadParser printGenericDebugLog:@"Base banner: 'ctaDirection' is a NSString but neither 'h' or 'v': ignoring."];
        }
    }
                                   ...
                                   withBaseObject:(BAMSGMessageImage* _Nonnull)image
{
    ...
    image.allowSwipeToDismiss = [[json objectForKey:@"swipe_dismiss" kindOfClass:[NSNumber class] fallback:@(true)] boolValue];
    ...
    // fallback on CGSizeZero if no width or height are specified. Message presenter will morph to the right size upon image download.
    ...
    if (err != nil) {
        [BAMSGPayloadParser printGenericDebugLog:[NSString stringWithFormat:@"Image: 'action' couldn't be parsed properly. Err: %@", err]];
        return nil;
    }
                                       ...
                                       withBaseObject:(BAMSGMessageWebView* _Nonnull)webViewMessage
{
    ...
    if (err != nil) {
        [BAMSGPayloadParser printGenericDebugLog:[NSString stringWithFormat:@"Image: 'url' couldn't be parsed properly. Err: %@", err]];
        return nil;
    }
    ...
    if (![@"http" isEqualToString:scheme] && ![@"https" isEqualToString:scheme]) {
        [BAMSGPayloadParser printGenericDebugLog:[NSString stringWithFormat:@"Image: 'url' is not valid: only HTTP and HTTPS URL schemes are supported."]];
        return nil;
    }
...
+ (BAMSGAction* _Nullable)parseGlobalAction:(NSDictionary* _Nonnull)userData
{
    ...
    if (actionString != nil && (![actionString isKindOfClass:[NSString class]] || [actionString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length == 0))
    {
...
+ (BAMSGCTA* _Nullable)parseCTA:(NSDictionary* _Nonnull)userData
{
    ...
    if (![label isKindOfClass:[NSString class]] || [label stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length == 0)
    {
    ...
    if (action != nil && (![action isKindOfClass:[NSString class]] || [action stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length == 0))
    {
