...
if (attrValue == nil) { \
    *error = [self logAndMakeSaveErrorWithCode:BatchUserDataEditorErrorInvalidValue \
                                        reason:@"The attribute's value cannot be nil. Did you mean to use 'removeAttributeForKey'?"]; \
    return false; \
} \
if (![attrValue isKindOfClass:expectedClass]) { \
    *error = [self logAndMakeSaveErrorWithCode:BatchUserDataEditorErrorInvalidValue \
                                        reason:@"The attribute's value isn't of the expected class (%@)", NSStringFromClass(expectedClass)]; \
    return false; \
} \
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        dispatch_once(&onceToken, ^{
            NSError *error = nil;
            regex = [NSRegularExpression regularExpressionWithPattern:ATTRIBUTE_NAME_RULE
                                                              options:0
                                                                error:&error];
            if (error)
            {
...
- (void)setLanguage:(nullable NSString*)language
{
    if (![BANullHelper isNull:language] && [language isKindOfClass:[NSString class]]) {
        if ([language length] < 2) {
            [BALogger publicForDomain:PUBLIC_DOMAIN message:@"setLanguage called with invalid language (must be at least 2 chars)"];
            return;
        } else if ([language length] > 128) {
            [BALogger publicForDomain:PUBLIC_DOMAIN message:@"setLanguage called with invalid language (must be less than 128 chars)"];
            return;
        }
    }
...
- (void)setRegion:(nullable NSString*)region
{
    if (![BANullHelper isNull:region] && [region isKindOfClass:[NSString class]]) {
        if ([region length] < 2) {
            [BALogger publicForDomain:PUBLIC_DOMAIN message:@"setRegion called with invalid region (must be at least 2 chars)"];
            return;
        } else if ([region length] > 128) {
            [BALogger publicForDomain:PUBLIC_DOMAIN message:@"setRegion called with invalid region (must be less than 128 chars)"];
            return;
        }
    }
...
{
    if (![BANullHelper isNull:identifier] && [identifier isKindOfClass:[NSString class]] && [identifier length] > 1024) {
        [BALogger publicForDomain:PUBLIC_DOMAIN message:@"setIdentifier called with identifier region (must be less 1024 chars)"];
        return;
    }
...
- (void)setAttribute:(nullable NSObject*)attribute forKey:(nonnull NSString*)key
{
    ...
    [BALogger publicForDomain:PUBLIC_DOMAIN message:@"Invalid attribute value. Please check the documentation for accepted values. Ignoring attribute '%@'.", key];
    return;
}
...
- (BOOL)setStringAttribute:(nonnull NSString*)attribute forKey:(nonnull NSString*)key error:(NSError * _Nullable * _Nullable)error
{
    INIT_AND_BLANK_ERROR_IF_NEEDED(error)
    VALIDATE_ATTRIBUTE_KEY_OR_BAIL()
    ...
    {
        *error = [self logAndMakeSaveErrorWithCode:BatchUserDataEditorErrorInvalidValue
                                            reason:@"String attributes can't be longer than %d characters. Ignoring attribute '%@'.", ATTR_STRING_MAX_LENGTH, key];
        return false;
    }
...
- (BOOL)setNumberAttribute:(nonnull NSNumber*)numberAttr forKey:(nonnull NSString*)key error:(NSError **)error
{
    ...
    *error = [self logAndMakeSaveErrorWithCode:BatchUserDataEditorErrorInvalidValue
                                        reason:@"Unsupported NSNumber type. Ignoring attribute '%@' for value '%@'.", key, numberAttr];
    return false;
}
...
- (void)addTag:(nonnull NSString*)tag inCollection:(nonnull NSString*)collection
{
    ...
    if (!didTagValidate)
    {
        [BALogger publicForDomain:PUBLIC_DOMAIN message:@"Invalid tag. Please make sure that the tag is a non empty string. It also can't be longer than %d characters. Ignoring tag '%@' for collection '%@'.", ATTR_STRING_MAX_LENGTH, tag, collection];
        return;
    }
...
- (void)save:(void (^)(void))completion
{
    @synchronized(_operationQueue) {
        ...
        dispatch_async([BAUserDataManager sharedQueue], ^{
            ...
            BAUserAttributesDiff *attributesDiff = [[BAUserAttributesDiff alloc] initWithNewAttributes:newAttributes previous:oldAttributes];
            BAUserTagCollectionsDiff *tagCollectionsDiff = [[BAUserTagCollectionsDiff alloc] initWithNewTagCollections:newTagCollections previous:oldTagCollections];
...
- (BOOL)canSave
{
    if (![[[BACoreCenter instance] status] isRunning])
    {
        [BALogger publicForDomain:PUBLIC_DOMAIN
                          message:@"Batch must be started before changes to user data can be saved. The changes you've just tried to save have been discarded."];
        return false;
    }
...
- (BOOL)writeChangesToDatasource:(NSArray<BOOL (^)(void)> *)applyQueue changeset:(long long)changeset {
    ...
    {
        [BALogger publicForDomain:PUBLIC_DOMAIN message:@"An internal error occurred while applying the changes. (Error code 35)"];
        return false;
    }
    ...
    for (BOOL (^operation)(void) in applyQueue)
    {
        if (!operation())
        {
            ...
            [BALogger publicForDomain:PUBLIC_DOMAIN message:@"An internal error occurred while applying the changes. (Error code 36)"];
            return false;
        }
    }
    ...
    {
        [BALogger publicForDomain:PUBLIC_DOMAIN message:@"An internal error occurred while applying the changes. (Error code 37)"];
        return false;
    }
...
{
   if (!_updatedFields[LANGAGUE_INDEX] &&
            !_updatedFields[REGION_INDEX] &&
            !_updatedFields[IDENTIFIER_INDEX])
    {
        // Nothing to do
        return nil;
    }
    ...
    return ^{
        ...
        bool updated = false;
        for (int i = 0; i < 3; ++i) {
            if (previousUserFields[i] != self->_userFields[i] && ![previousUserFields[i] isEqualToString:self->_userFields[i]]) {
                // Field have changed
                updated = true;
            }
        }
...
- (BOOL)validateAttributeKey:(NSString*)key error:(NSError * _Nullable * _Nonnull)error
{
    ...
    *error = [self logAndMakeSaveErrorWithCode:BatchUserDataEditorErrorInvalidKey
                                        reason:@"Invalid key. Please make sure that the key is made of letters, underscores and numbers only (a-zA-Z0-9_). It also can't be longer than 30 characters. Ignoring attribute '%@'.", key];
                     ...
                     operationErrorDescription:(NSString*)descriptionFormatString,...
{
    ...
    {
        NSRange matchingRange = [_attributeNameValidationRegexp rangeOfFirstMatchInString:collection
                                                                                  options:0
    ...
    va_end(arglist);
    *error = [self logAndMakeSaveErrorWithCode:BatchUserDataEditorErrorInvalidKey
                                        reason:@"Invalid collection. Please make sure that the collection is made of letters, underscores and numbers only (a-zA-Z0-9_). It also can't be longer than 30 characters. Ignoring %@.", operationDescription];
