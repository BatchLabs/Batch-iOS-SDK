...
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_MSEC)), [BAUserDataManager sharedQueue], ^{
        id<BAUserDatasourceProtocol> database = [BAInjection injectProtocol:@protocol(BAUserDatasourceProtocol)];
        
        if (database == nil) {
            [BALogger errorForDomain:@"BAUserDataManager" message:@"Could not send attributes to backend, missing database."];
            return;
        }
        ...
        BAQueryWebserviceClient *ws = [[BAQueryWebserviceClient alloc] initWithDatasource:wsDatasource
...
+ (void)storeTransactionID:(NSString*)transaction forVersion:(NSNumber*)version
{
    ...
    dispatch_async([BAUserDataManager sharedQueue], ^{
        ...
        {
            [BALogger debugForDomain:@"BAUserDataManager" message:@"Wrong changeset (ours: %@, server %@), resending", changeset, version];
            // Send in 15s
