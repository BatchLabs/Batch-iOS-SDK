...
{    
    ...
    */
    ...
    {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Error while opening sqlite database, not persisting user data."];
        return nil;
    }
    ...
    BOOL attributesCreated = [self executeSimpleStatement:[NSString stringWithFormat:@"create table if not exists %@ (name text not null, type integer, value text, changeset integer, unique(name) on conflict replace, unique(name, type, value) on conflict abort);", TABLE_ATTRIBUTES]];
    
    BOOL tagsCreated = [self executeSimpleStatement:[NSString stringWithFormat:@"create table if not exists %@ (collection text not null, value text not null, changeset integer, unique(collection, value) on conflict abort);", TABLE_TAGS]];
    ...
    NSString *statement = [NSString stringWithFormat:@"INSERT INTO %@ (name, type, value, changeset) VALUES (?, ?, ?, ?)", TABLE_ATTRIBUTES];
    
    if( sqlite3_prepare_v2(_database, [statement cStringUsingEncoding:NSUTF8StringEncoding], -1, &_attributeInsertStatement, NULL) != SQLITE_OK )
    {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Error while preparing the attribute sqlite insert statement, not persisting data."];
        return nil;
    }
    
    statement = [NSString stringWithFormat:@"INSERT INTO %@ (collection, value, changeset) VALUES (?, ?, ?)", TABLE_TAGS];
    
    if( sqlite3_prepare_v2(_database, [statement cStringUsingEncoding:NSUTF8StringEncoding], -1, &_tagInsertStatement, NULL) != SQLITE_OK )
    {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Error while preparing the tag sqlite insert statement, not persisting data."];
        return nil;
    }
    
    statement = [NSString stringWithFormat:@"DELETE FROM %@ WHERE name=?", TABLE_ATTRIBUTES];
    
    if( sqlite3_prepare_v2(_database, [statement cStringUsingEncoding:NSUTF8StringEncoding], -1, &_attributeDeleteStatement, NULL) != SQLITE_OK )
    {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Error while preparing the sqlite delete statement, not persisting data."];
        return nil;
    }
  
    statement = [NSString stringWithFormat:@"DELETE FROM %@ WHERE collection=?", TABLE_TAGS];
    
    if( sqlite3_prepare_v2(_database, [statement cStringUsingEncoding:NSUTF8StringEncoding], -1, &_tagCollectionDeleteStatement, NULL) != SQLITE_OK )
    {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Error while preparing the sqlite delete statement, not persisting data."];
        return nil;
    }
    ...
    if( sqlite3_prepare_v2(_database, [statement cStringUsingEncoding:NSUTF8StringEncoding], -1, &_tagDeleteStatement, NULL) != SQLITE_OK )
    {
        [BALogger errorForDomain:LOGGER_DOMAIN message:@"Error while preparing the sqlite delete statement, not persisting data."];
        return nil;
    }
...
- (BOOL)setLongLongAttribute:(long long)attribute forKey:(NSString*)key
{
    ...
    } forKey:key withType:BAUserAttributeTypeLongLong isNative:NO];
}
...
- (BOOL)setDoubleAttribute:(double)attribute forKey:(NSString*)key
{
    ...
    } forKey:key withType:BAUserAttributeTypeDouble isNative:NO];
}
...
- (BOOL)setBoolAttribute:(BOOL)attribute forKey:(NSString*)key
{
    ...
    } forKey:key withType:BAUserAttributeTypeBool isNative:NO];
}
...
- (BOOL)setStringAttribute:(NSString*)attribute forKey:(NSString*)key
{
    ...
    } forKey:key withType:BAUserAttributeTypeString isNative:NO];
}
...
- (BOOL)clearTagsFromCollection:(NSString*)collection
{
    ...
    sqlite3_bind_text(_tagCollectionDeleteStatement, 1, [collection cStringUsingEncoding:NSUTF8StringEncoding], -1, NULL);
...
{
    NSString *selectSQL = [NSString stringWithFormat:@"SELECT collection, value FROM %@ ORDER BY collection", TABLE_TAGS];
    ...
    if( sqlite3_prepare_v2(self->_database, [selectSQL cStringUsingEncoding:NSUTF8StringEncoding], -1, &statement, NULL) == SQLITE_OK )
    {
        ...
        while( sqlite3_step(statement) == SQLITE_ROW )
        {
            NSString *collection = [NSString stringWithCString:(const char*) sqlite3_column_text(statement, 0) encoding:NSUTF8StringEncoding];
            NSString *tag = [NSString stringWithCString:(const char*) sqlite3_column_text(statement, 1) encoding:NSUTF8StringEncoding];
...
- (NSString*)printDebugDump
{
    ...
    NSString *dump = [NSString stringWithFormat:@"%@Attributes: %@\n\nTags: %@", baseString, [BAUserAttribute serverJsonRepresentationForAttributes:[self attributes]], [self tagCollections]];
...
- (BOOL)setAttributeUsingBlock:(void (^)(sqlite3_stmt *statement, int columnNumber))bindBlock forKey:(NSString*)key withType:(BAUserAttributeType)type isNative:(BOOL)native
{
