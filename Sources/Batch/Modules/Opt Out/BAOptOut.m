...
- (void)setOptedOut:(BOOL)shouldOptOut wipeData:(BOOL)wipeData completionHandler:(BatchOptOutNetworkErrorPolicy(^ _Nonnull)(BOOL success))completionHandler
{
    ...
    [BALogger debugForDomain:@"OptOut" message:@"Setting opt-out value: %@, wipe data: %@", shouldOptOut ? @"true":@"false", wipeData ? @"true":@"false"];
    ...
    if (shouldOptOut != _optedOut) {
        ...
        // Prepare the operation block. If there is a completion handler, and the user asked for an opt out, wait for the event promise
        // Otherwise, don't wait for it
        
        BAPromise *waitPromise = nil;
        if (shouldOptOut && wipeData) {
            waitPromise = [_eventTracker track:[BAEvent eventWithName:@"_OPTOUT_WIPE_DATA" andParameters:[self makeBaseEventData]]];
        } else if (shouldOptOut) {
            waitPromise = [_eventTracker track:[BAEvent eventWithName:@"_OPTOUT" andParameters:[self makeBaseEventData]]];
        }
        
        if (waitPromise != nil && completionHandler != nil) {
            [BALogger debugForDomain:@"OptOut" message:@"Waiting for server and developer response before optin-out and/or wiping data"];
...
- (void)applyOptOut:(BOOL)shouldOptOut wipeData:(BOOL)wipeData
{
    ...
    [[BANotificationCenter defaultCenter] postNotificationName:kBATOptOutChangedNotification object:nil userInfo:@{kBATOptOutValueKey: @(shouldOptOut), kBATOptOutWipeDataKey: @(wipeData)}];
...
- (BOOL)readOptOutFromDisk
{
    ...
    if ([optOut isKindOfClass:[NSNumber class]]) {
        ...
        [BALogger debugForDomain:@"OptOut" message:@"No opt out value, reading BATCH_OPTED_OUT_BY_DEFAULT from plist: %@", infoPlistOptout];
