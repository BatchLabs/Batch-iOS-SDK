...
- (void)handleResponse:(NSDictionary *)response
{
    ...
    {
        [BALogger errorForDomain:@"Webservice" message:@"Query Webservice returned an invalid response: %@", [re description]];
    ...
    for (id<BAWSQuery> query in self.sentQueries)
    {
        ...
        for (NSDictionary *response in webserviceResponses)
        {
            if (![BANullHelper isDictionaryEmpty:response])
            {
                ...
                if (![BANullHelper isStringEmpty:responseIdentifier] && [queryIdentifier isEqualToString:responseIdentifier])
                {
...
- (NSMutableDictionary*)requestBodyDictionary
{
    ...
    if (![BANullHelper isArrayEmpty:queriesToSend]) {
        ...
        [BALogger errorForDomain:LOCAL_ERROR_DOMAIN message:@"Query WS will be send without any query. URL: %@", self.url];
    }
    ...
    NSDictionary *notificationAuthorization = [[[[[BACoreCenter instance] status] notificationAuthorization] currentSettings] optionalDictionaryRepresentation];
    ...
    {
        [BALogger errorForDomain:@"BatchWebserviceMetrics" message:@"Error while adding metrics to webservice: %@", [metricException description]];
    }
...
- (void)connectionFailedWithError:(NSError *)error
{
    ...
    [BALogger errorForDomain:LOCAL_ERROR_DOMAIN message:@"%@ webservice return an error: %@", self.datasource.requestIdentifier,[error description]];
...
- (void)connectionDidFinishLoadingWithData:(NSData *)data
{
    ...
    {
        [self updateMetricWithSuccess:false];
        [BALogger errorForDomain:LOCAL_ERROR_DOMAIN message:@"%@ webservice return a NULL or empty data.", self.datasource.requestIdentifier];
        [self.delegate webserviceClient:self didFailWithError:[BAErrorHelper webserviceError]];
        return;
    }
    ...
    @try
    {
        ...
        {
            [[NSException exceptionWithName:@"Invalid content." reason:[NSString stringWithFormat:@"%@ response is NULL or empty.", self.datasource.requestIdentifier] userInfo:nil] raise];
        }
    }
    @catch (NSException *exception)
    {
        [self updateMetricWithSuccess:false];
        [BALogger errorForDomain:LOCAL_ERROR_DOMAIN message:@"Error %@ webservice: %@", self.datasource.requestIdentifier, [exception description]];
        [self.delegate webserviceClient:self didFailWithError:[BAErrorHelper webserviceError]];
    }
}
