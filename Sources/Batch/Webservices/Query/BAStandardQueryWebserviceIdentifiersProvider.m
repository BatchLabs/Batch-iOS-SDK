...
- (NSDictionary<NSString*, NSString*>*)identifiers
{
    ...
    NSString *advancedIdsList = [BAParameter objectForKey:kParametersAdvancedIDsPatternKey fallback:kParametersAdvancedIDsPatternValue];
    NSArray *advancedIds = nil;
    
    if (![BANullHelper isStringEmpty:advancedIdsList] && [[BACoreCenter instance].configuration useAdvancedDeviceInformation]) {
        advancedIds = [advancedIdsList componentsSeparatedByString:@","];
    }
    
    NSMutableArray<NSString*> *idsToFetch = [[NSMutableArray alloc] initWithCapacity:(baseIds.count + advancedIds.count)];
    if (baseIds) {
