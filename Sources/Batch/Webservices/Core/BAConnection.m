...
- (nonnull instancetype)initWithSession:(nonnull id<BAURLSessionProtocol>)session contentType:(BAConnectionContentType)contentType delegate:(nullable id<BAConnectionDelegate>)delegate
{
    self = [super init];
    if (self)
    {
...
- (void)start
{
    if ([[BAOptOut instance] isOptedOut] && !self.canBypassOptOut) {
        [BALogger debugForDomain:@"BAConnection" message:@"Refusing to execute webservice client, as Batch is opted-out from, and webservice isn't whitelisted"];
        
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [self.delegate connectionDidFinishSuccessfully:NO];
        });
        
        NSError *err = [NSError errorWithDomain:NETWORKING_ERROR_DOMAIN code:BAConnectionErrorCauseOptedOut userInfo:nil];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
- (void)handleDataTaskResponse:(nullable NSURLResponse*)response data:(nullable NSData*)data error:(nullable NSError*)error
{
    if (response == nil && error == nil) {
        error = [NSError errorWithDomain:NETWORKING_ERROR_DOMAIN code:BAConnectionErrorCauseOther userInfo:@{@"subcode": @1}];
        goto bail_on_err;
    }
    ...
    if (error == nil) {
        ...
        if (![response isKindOfClass:[NSHTTPURLResponse class]]) {
            error = [NSError errorWithDomain:NETWORKING_ERROR_DOMAIN code:BAConnectionErrorCauseOther userInfo:@{@"subcode": @2}];
            goto bail_on_err;
        }
        ...
        if (data == nil) {
            ...
            error = [NSError errorWithDomain:NETWORKING_ERROR_DOMAIN code:BAConnectionErrorCauseOther userInfo:@{@"subcode": @4}];
            goto bail_on_err;
        }
        
        if (_cryptorFactory != nil) {
            id<BAWebserviceCryptor> cryptor = [_cryptorFactory inboundCryptorForData:data connection:self response:httpResponse];
            data = [cryptor decrypt:data];
            
            if (data == nil) {
                // We should NOT have a nil data at this point
                error = [NSError errorWithDomain:NETWORKING_ERROR_DOMAIN code:BAConnectionErrorCauseOther userInfo:@{@"subcode": @5}];
                goto bail_on_err;
            }
        }
