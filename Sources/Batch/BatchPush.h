...
/**
 ...
 @deprecated See BatchPushOpenedNotification
 */
FOUNDATION_EXPORT NSString * _Nonnull const BatchPushReceivedNotification __attribute__((deprecated("Use BatchPushOpenedNotification, which is more predictable.")));

/**
 Notification sent by Batch Push when a notification has been opened by the user.
 Background wakeups and notifications received while in the foreground (that are not presented to the user) will not trigger this notification.
 If your application doesn't properly call Batch in your UNUserNotificationDelegate or doesn't setup any delegate at all, this notification WILL misbehave:
  It might miss opens, or report background fetchs as opens.
 
 This notification's userInfo contains private keys that are not guaranteed to be the same between SDK releases, but you can use BatchPushOpenedNotificationPayloadKey to get the payload.
...
/**
 ...
 @deprecated
 */
+ (void)registerForRemoteNotificationsWithCategories:(nullable NSSet *)categories NS_AVAILABLE_IOS(8_0) __attribute__((deprecated("Use setNotificationCategories and registerForRemoteNotifications separately.")));
...
@param userInfo The untouched "userInfo" NSDictionary argument given to you in the application delegate method.
*/
+ (void)handleNotification:(nonnull NSDictionary*)userInfo __attribute__((deprecated("Implement UNUserNotificationCenterDelegate using BatchUNUserNotificationCenterDelegate or your own implementation")));
 ...
 */
+ (void)handleNotification:(nonnull NSDictionary*)userInfo actionIdentifier:(nullable NSString*)identifier NS_AVAILABLE_IOS(8_0);
 ...
 */
+ (void)handleRegisterUserNotificationSettings:(nonnull UIUserNotificationSettings *)notificationSettings __attribute__((deprecated("Use BatchPush.requestNotificationAuthorization()")));
...
/**
 ...
 @param willShowSystemForegroundAlert   Whether you will tell the framework to show this notification, or not. Batch uses this value to adjust its behaviour accordingly for a better user experience. Return 'true' if you're returning .alert (or more) to iOS' completion handler.
 */
+ (void)handleUserNotificationCenter:(nonnull UNUserNotificationCenter *)center willPresentNotification:(nonnull UNNotification *)notification willShowSystemForegroundAlert:(BOOL)willShowSystemForegroundAlert NS_AVAILABLE_IOS(10_0) NS_SWIFT_NAME(handle(userNotificationCenter:willPresent:willShowSystemForegroundAlert:));
 ...
 */
+ (void)handleUserNotificationCenter:(nonnull UNUserNotificationCenter *)center didReceiveNotificationResponse:(nonnull UNNotificationResponse *)response NS_AVAILABLE_IOS(10_0) NS_SWIFT_NAME(handle(userNotificationCenter:didReceive:));
...
/// Equivalent to calling [UNUserNotificationCenter currentNotificationCenter].delegate = BatchUNUserNotificationCenterDelegate.sharedInstance
