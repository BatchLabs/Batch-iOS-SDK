...
#pragma clang diagnostic pop
    ...
    response = [[BAWSResponse alloc]
        initWithResponse:@{@"id" : @"RESPONSE", @"code" : @"CODE", @"infos" : @{@"type" : @"INVALID_CODE"}}];
    XCTAssertNotNil(response, @"Cannot creat a response with a valid input.");
    XCTAssert([@"RESPONSE" isEqualToString:response.reference], @"Invalid response reference");
}
...
- (void)testErrors {
    ...
    NSDictionary *SUCCESS_COMPLETE_JSON = @{
        @"id" : @"abc",
        @"code" : @"code",
        @"status" : @"SUCCESS",
        @"infos" : [NSNull null],
        @"offer" : @{
            @"r" : @"promotion1",
            @"id" : @"promo1",
            @"data" : @[ @{@"n" : @"key1", @"v" : @"value1"} ],
            @"tok" : @"token2",
            @"bundles" : @[ @{
                @"r" : @"bundle1",
                @"feat" : @[ @{@"r" : @"feature1", @"val" : @"value1"} ],
                @"res" : @[ @{@"r" : @"res1", @"val" : @10} ]
            } ],
            @"feat" : @[ @{@"r" : @"feature2", @"val" : @"value2"} ],
            @"res" : @[ @{@"r" : @"res2", @"val" : @10} ]
        }
    };
