...
public func XCTAssertResolves<T>(_ expected: @autoclosure () throws -> T?, _ promise: @autoclosure () throws -> BAPromise<T>, _ message: @autoclosure () -> String = "", file: StaticString = #filePath, line: UInt = #line) where T : Equatable {
    ...
    promise.catch { (err) in
        XCTFail("Promise was expected to be resolved with '\(String(describing: expected))' but was rejected with '\(String(describing: err))'.", file: file, line: line)
    }
}
...
public func XCTAssertRejects<T>(_ promise: @autoclosure () throws -> BAPromise<T>, _ message: @autoclosure () -> String = "", file: StaticString = #filePath, line: UInt = #line) {
    let promise = try! promise()
    promise.then { (_ resolvedValue: T?) in
...
public func XCTAssertNoPendingPromise<T>(_ promise: @autoclosure () throws -> BAPromise<T>, file: StaticString = #filePath, line: UInt = #line) {
    let promise = try! promise()
...
// This should be ran at the end of your test
public func XCTAssertNoPendingPromise<T>(_ promises: @autoclosure () throws -> [BAPromise<T>], file: StaticString = #filePath, line: UInt = #line) {
