...
- (void)testDates {
    ...
    XCTAssertTrue([value rangeOfCharacterFromSet:[NSCharacterSet characterSetWithCharactersInString:@",pam "]
                                         options:NSCaseInsensitiveSearch]
                          .location == NSNotFound,
                  @"Properties center returned an invalid date.");
...
- (void)testNotifType {
    ...
    XCTAssertEqualObjects(@"fallback", [properties notifType],
                          @"Expected fallback when BANotificationAuthorization is waiting for a value");

    authSettings.status = BANotificationAuthorizationStatusDenied;

    XCTAssertEqualObjects(@"0", [properties notifType],
                          @"Expected 0 when BANotificationAuthorization is not requested");
    ...
    authSettings.types = BANotificationAuthorizationTypesAlert | BANotificationAuthorizationTypesNotificationCenter;
    XCTAssertEqualObjects(expectedAlert, [properties notifType]);

    authSettings.types = BANotificationAuthorizationTypesAlert | BANotificationAuthorizationTypesLockscreen;
    XCTAssertEqualObjects(expectedAlert, [properties notifType]);
    ...
    authSettings.types =
        BANotificationAuthorizationTypesLockscreen | BANotificationAuthorizationTypesNotificationCenter;
    XCTAssertEqualObjects(expectedAlert, [properties notifType]);
    ...
    authSettings.types = BANotificationAuthorizationTypesBadge | BANotificationAuthorizationTypesSound |
                         BANotificationAuthorizationTypesAlert | BANotificationAuthorizationTypesLockscreen |
                         BANotificationAuthorizationTypesNotificationCenter;
    XCTAssertEqualObjects(@"7", [properties notifType]);

    authSettings.types = BANotificationAuthorizationTypesBadge | BANotificationAuthorizationTypesSound |
                         BANotificationAuthorizationTypesAlert | BANotificationAuthorizationTypesNotificationCenter;
    XCTAssertEqualObjects(@"7", [properties notifType]);
