...
class eventTrackerServiceTests: XCTestCase {
    ...
    func testResponse() {
        ...
        XCTAssertTrue(service.response(for: BAWSQueryTracking(events: []), content: makeBasicQueryResponseDictionary()) is BAWSResponseTracking)
    }
    
    func testWebserviceFail() {
        let promises = [BAPromise(), BAPromise()]
        let service = makeServiceWithPromises(promises)
        service.webserviceClient(makeQueryClient(service: service), didFailWithError: NSError(domain: "test", code: 1, userInfo: nil))
    ...
    func testWebserviceSuccess() {
        let promises = [BAPromise(), BAPromise()]
        let service = makeServiceWithPromises(promises)
        service.webserviceClient(makeQueryClient(service: service), didSucceedWith: [BAWSResponseTracking(response: makeBasicQueryResponseDictionary())])
