...
class webserviceHMACTests: XCTestCase {
    ...
    func testURLRequest() {
        ...
        XCTAssertEqual("SHA256 content-type,foo,x-batch-cipher-version Gf8afLnF3TY/LqACciolOoALYeQSZ2dXy8Vxhfl7kI4=",
                       request.allHTTPHeaderFields?["X-Batch-Signature"] ?? "error")
        
        let body = "!&é\"'(§è!çà)-12567890°_%^$mù`=*/.,?,;:=‘{«ÇøÇø}—ë‘¶Ç¡@#|¿¡ïŒ€£µ~©®†™≈<>≤≥êÊ•π‡∂ƒÌ¬◊ß∞÷≠+∫√¢‰∆∑Ω¥∏ªŸ[]å”„ック金型илджفيحةحديد"
        ...
        XCTAssertEqual("SHA256 content-sha1,content-type,foo,x-batch-cipher-version +FZLG5QtIKKrW1zxwzboezVNAb5VJ5RZI0319Wrzal0=",
                       request.allHTTPHeaderFields?["X-Batch-Signature"] ?? "error")
    }
    
    func testHmac() {
        let precomputedHmacWithHeaders = "SHA256 content-type,foo,x-batch-cipher-version 8Qc8LvpoiakXejOZ9HzkxJOhLulKUdU3fzIL8dk720M=" // key: 12345
        ...
        XCTAssertEqual(precomputedHmacWithHeaders, hmac.hmac(forMethod: "post",
        ...
        XCTAssertEqual(precomputedHmacWithoutHeaders, hmac.hmac(forMethod: "post",
                                                                relativeURL: "/foo/BAR?param=value",
                                                                headers: [:]))
        
        XCTAssertEqual(precomputedHmacWithNoPath, hmac.hmac(forMethod: "post",
                                                            relativeURL: "",
                                                            headers: [:]))
    }
    ...
    func testRequestSummary() {
        XCTAssertEqual("""
                        ...
                        x-batch-cipher-version: 2
                        """,
                       hmac._requestSummary(forMethod: "post",
                                          relativeURL: "/foo/BAR?param=value",
                                              headers: [
                                                "foo": "bar",
                                                "x-batch-CIPHER-version": "2",
        ...
        XCTAssertEqual("POST /foo/BAR?param=value",
                       hmac._requestSummary(forMethod: "post",
                                          relativeURL: "/foo/BAR?param=value",
                                              headers: [:])
        )
    ...
    func testRelativeURLs() {
        ...
        XCTAssertEqual("/foo/bar?param1=value1&param2=value2", hmac._extractRelativeURL(URL(string: "https://batch.com/foo/bar?param1=value1&param2=value2")!))
    }
