...
class webviewBridgeLegacyTests: XCTestCase, BATWebviewBridgeLegacyWKHandlerWebViewSource {
    ...
    func testTaskIDParsing() {
        ...
        handler.userContentController(contentController, didReceive: MockWKMessage(body: [
            "taskID": "2",
            "method": "success",
            "args": [:]
        ]))
    ...
    func testJavascriptCallbackEvaluation() {
        ...
        webViewMock.it.expect().call(
            webViewMock.evaluateJavaScript(Arg.eq(makeJSEvalExpectationForSuccess(expected: "ok", taskID: taskID)),
        ...
        webViewMock.it.expect().call(
            webViewMock.evaluateJavaScript(Arg.eq(makeJSEvalExpectationForSuccess(expected: "test_value", taskID: taskID)),
                                           completionHandler: Arg.closure())
        )
        ...
        webViewMock.it.expect().call(
            webViewMock.evaluateJavaScript(Arg.eq(makeJSEvalExpectationForSuccess(expected: nil, taskID: taskID)),
                                           completionHandler: Arg.closure())
        )
        ...
        webViewMock.it.expect().call(
            webViewMock.evaluateJavaScript(Arg.eq(makeJSEvalExpectationForError(expected: bridge.expectedErrorMessage, taskID: taskID)),
                                           completionHandler: Arg.closure())
        )
