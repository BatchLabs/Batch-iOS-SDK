...
- (void)testPackUnpack {
    ...
    NSDictionary *ed = @{
        @"i": @"je-suis-un-send-id",
        @"e": @"je-suis-un-experiment-id",
        @"v": @"je-suis-un-varient-id"
    };
    
    BADisplayReceipt *receipt = [[BADisplayReceipt alloc] initWithTimestamp:123456 replay:false sendAttempt:19 openData:od eventData:ed];
...
-(void)testPackEmptyMap {
    BADisplayReceipt *receipt = [[BADisplayReceipt alloc] initWithTimestamp:65481651581 replay:true sendAttempt:6585 openData:@{} eventData:@{}];
    XCTAssertNotNil(receipt);
    NSError *error;
    NSData *packedData = [receipt pack:&error];
    XCTAssertNotNil(packedData);
...
- (void)testPackNil {
    BADisplayReceipt *receipt = [[BADisplayReceipt alloc] initWithTimestamp:65481651581 replay:true sendAttempt:6585 openData:nil eventData:nil];
    XCTAssertNotNil(receipt);
    NSError *error;
    NSData *packedData = [receipt pack:&error];
    XCTAssertNotNil(packedData);
