...
- (void)testMissingNotificationInCache
{
    ...
    NSArray<BAInboxCandidateNotification*> *candidates = [_datasource candidateNotificationsFromCursor:cursor limit:limit fetcherId:fetcherId];
    ...
    void (^successHandler)(BAInboxWebserviceResponse* _Nonnull) = ^(BAInboxWebserviceResponse* _Nonnull response) {
        ...
        if (sqlite3_prepare_v2(self->_database, [selectQuery cStringUsingEncoding:NSUTF8StringEncoding], -1, &selectStatement, NULL) == SQLITE_OK)
        {
            ...
            while (sqlite3_step(selectStatement) == SQLITE_ROW) {
                ...
                // notification_id
                XCTAssertEqualObjects(@"test-id-2.5", [NSString stringWithUTF8String: (const char *) sqlite3_column_text(selectStatement, 2)]);
                // install_id
                XCTAssertEqualObjects(@"b5baf3e0-a01f-11ea-111a-17c13e111be2", [NSString stringWithUTF8String:(const char *) sqlite3_column_text(selectStatement, 3)]);
                // custom_id
    ...
    BAWebserviceClient *client = [[BAInboxSyncWebserviceClient alloc] initWithIdentifier:@"test-custom-id"
...
- (void)testUpdatingNotificationInCache
{
    ...
    void (^successHandler)(BAInboxWebserviceResponse* _Nonnull) = ^(BAInboxWebserviceResponse* _Nonnull response) {
        ...
        XCTAssertEqualObjects(@"test-install-id-1-updated", [[response notifications] objectAtIndex:1].identifiers.installID);
        XCTAssertEqualObjects(@"test-custom-id-1-updated", [[response notifications] objectAtIndex:1].identifiers.customID);
    ...
    NSString* payload = @"{\"notifications\":[{\"read\":true,\"notificationId\":\"test-id-2\"},{\"installId\":\"test-install-id-1-updated\",\"customId\":\"test-custom-id-1-updated\",\"notificationId\":\"test-id-1\",\"notificationTime\":7300000,\"sendId\":\"test-send-id-1-updated\"}],\"hasMore\":true,\"timeout\":false,\"cursor\":\"test-id-1\"}";
    NSData* data = [payload dataUsingEncoding:NSUTF8StringEncoding];
    [client connectionDidFinishLoadingWithData:data];
}
