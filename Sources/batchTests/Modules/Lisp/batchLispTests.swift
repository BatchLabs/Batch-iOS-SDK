...
class batchLispTests: XCTestCase {
    ...
    func testBasicAST() {
        assert(expression: "(= 1 2)", parsesTo: [
            MockOperator("="),
            PrimitiveValue(double: 1),
            PrimitiveValue(double: 2),
        ])
        
        assert(expression: "(= true (> 1 2))", parsesTo: [
            MockOperator("="),
        ...
        assert(expression: "(= `foo` \"bar\")", parsesTo: [
            MockOperator("="),
            VariableValue.variable(withName: "foo"),
            PrimitiveValue(string: "bar")!,
        ])
        
        assert(expression: "(= true \"bar\\n\\t\\r\\\"\\\'\\\\ \")", parsesTo: [
            MockOperator("="),
            PrimitiveValue.init(boolean: true),
            PrimitiveValue(string: "bar\n\t\r\"\'\\ ")!,
        ])
        
        assert(expression: "(= true [\"a\" \"b\"])", parsesTo: [
            MockOperator("="),
            PrimitiveValue.init(boolean: true),
        ...
        assert(expression: "(= true [\"b\" \"a\" \"b\"])", parsesTo: [
            MockOperator("="),
            PrimitiveValue.init(boolean: true),
            PrimitiveValue.init(stringSet: ["a", "b"])!
            ])
    ...
    func assertBoolean(_ expression: String, expectedResult: Bool, file: StaticString = #filePath, line: UInt = #line) {
        ...
        case let error as BALErrorValue:
            if error.kind == .typeParser {
                XCTFail("Expression should be \(expectedResult), but could not be parsed: \(expression) Error: \(error)", file: file, line: line)
                return
            }
            XCTFail("Expression should be \(expectedResult), but errorred: \(expression) Error: \(error)", file: file, line: line)
        case let result as BALPrimitiveValue:
            ...
            if let boolValue = (result.value as? NSNumber)?.boolValue {
                XCTAssert(boolValue == expectedResult, "Expression result should be \(expectedResult): \(expression)", file: file, line: line)
                return
            }
            
            XCTFail("Expression result is of type bool, but the actual value could not be extracted. Looks like an internal error", file: file, line: line)
        default:
            XCTFail("Expression should be \(expectedResult), but executing it resulted in an unexpected value: \(expression)", file: file, line: line)
        }
    }
    ...
    func assert(expression: String, parsesTo expectedAST: LiteralSExpression, file: StaticString = #file, line: UInt = #line) {
    ...
    func assert(expression: String, parsesTo expectedAST: BALValue, file: StaticString = #filePath, line: UInt = #line) {
        ...
        XCTAssertTrue(parsedExpression.isEqual(expectedAST), "Expression \(expression) does not parses to expected AST: \(expectedAST)", file: file, line: line)
    }
